 ...
 */
export type ProviderLogoEdit =
  { kind: 'image'; file: File } | { kind: 'key'; key: string } | { kind: 'default' }
...
export function useProviderEditor({ onProviderCreated }: UseProviderEditorParams) {
  ...
  const startAddFrom = useCallback(
    (source: Provider) => updateMode({ kind: 'duplicate', source }),
    [updateMode]
  )
  const startEdit = useCallback(
    (provider: Provider) => updateMode({ kind: 'edit', provider }),
    [updateMode]
  )
