...
export type CustomProviderCreationInvalidUrl =
  { field: 'textEndpointRequired' } | { field: 'endpointUrl'; endpointType: CustomProviderEndpoint }
...
export function buildCustomProviderCreationPayload(
  ...
  return {
    defaultChatEndpoint: getCustomProviderDefaultChatEndpoint(
      input.endpointUrls,
      input.preferredChatEndpoint
    ),
    endpointConfigs
  }
}
...
export function buildCustomProviderEndpointPreview(
  ...
  const formattedHost =
    endpointType === ENDPOINT_TYPE.GOOGLE_GENERATE_CONTENT
      ? formatApiHost(value, true, 'v1beta')
      : formatApiHost(value)
...
export function findInvalidCustomProviderCreationUrl(
  ...
  if (
    !CUSTOM_PROVIDER_TEXT_ENDPOINTS.some((endpointType) => trim(input.endpointUrls[endpointType]))
  ) {
