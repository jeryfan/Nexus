...
export default function ProviderList({ selectedProviderId, onSelectProvider }: ProviderListProps) {
  ...
  const [filterMode, setFilterMode] = useState<ProviderFilterMode>('all')
  ...
  }, [filterMode, providers, providerModelsIndex, searchText])
  ...
  const groupedPresetIds = useMemo(
    () => getGroupedPresetIds(filteredProviders),
    [filteredProviders]
  )
  ...
  useEffect(() => {
    ...
    const scrollSelectedItem = () => {
      ...
      const isFullyVisible =
        itemRect.top >= scrollerRect.top && itemRect.bottom <= scrollerRect.bottom
  ...
  const renderProviderItem = (
    provider: Provider,
    _index: number,
    state: ProviderListContentItemState
  ) => {
    const showManagementActions =
      (providerCounts.get(provider.id) ?? 0) > 1 || canManageProvider(provider)
  ...
  const handleAddAnother = useCallback(
    (template: Provider) => startAddFrom(template),
    [startAddFrom]
  )
