import { ReorderableList, Sortable } from '@nexus/ui'
...
function reorderProviderBlocks({
  ...
  const targetIndexes = remaining.flatMap((provider, index) =>
    targetIds.has(provider.id) ? [index] : []
  )
...
export default function ProviderListContent({
  ...
  const updateBottomAddVisibility = useCallback(() => {
    ...
    const shouldShow =
      ...
      !!scroller &&
      !!mainContent &&
      mainContent.scrollHeight > scroller.clientHeight + 1
  ...
  const renderGrouped = () => {
    ...
    const handleSortEnd = ({ oldIndex, newIndex }: { oldIndex: number; newIndex: number }) => {
      const nextProviders = reorderProviderBlocks({
        providers,
        items: sortableItems,
        oldIndex,
        newIndex
    ...
    return (
      <Sortable
        ...
        renderItem={(item, state) => {
          ...
          const expanded =
            !state.overlay && (searchActive || (expandedGroups[item.presetProviderId] ?? false))
          const containsSelected =
            !!selectedProviderId && item.members.some((m) => m.id === selectedProviderId)
  ...
  return (
    <Scrollbar ref={setScrollbarRef} className={providerListClasses.scroller}>
      <div className={providerListClasses.sectionStack}>
        <div
          ref={mainContentRef}
          className={providerListClasses.sectionStack}
          data-provider-list-main-content
        >
          {addButton}
          {hasResults ? (
            <section className={providerListClasses.section}>
              {hasAnyGroup ? renderGrouped() : renderFlat()}
            </section>
          ) : (
            <div className={providerListClasses.emptyState}>{'无结果'}</div>
          )}
        </div>
        {showBottomAddButton ? (
          <div className={providerListClasses.addBottomWrap}>{addButton}</div>
        ) : null}
      </div>
    </Scrollbar>
  )
