import { resolveProviderIconRef, useIcon } from '@nexus/ui/icons'
import {
  getIconDisplayConfig,
  type IconDisplayContext
} from '@renderer/components/icons/iconDisplayConfig'
...
export function ProviderAvatar({
  ...
  const displayIconId =
    typeof customLogo === 'string' && customLogo.startsWith('icon:')
      ? customLogo.slice('icon:'.length)
      : provider.id
  const displayConfig = displayContext
    ? getIconDisplayConfig(displayContext, displayIconId)
    : undefined
  const iconStyle: CSSProperties | undefined = displayConfig
    ? {
        ...
        flexShrink: 0,
        borderRadius:
          displayConfig.borderRadius === undefined ? undefined : `${displayConfig.borderRadius}px`,
