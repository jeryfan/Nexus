import { Input, PageSidePanelItem, Switch, Tooltip } from '@nexus/ui'
...
export default function ProviderApiOptionsDrawer({
  providerId,
  open,
  onClose
}: ProviderApiOptionsDrawerProps) {
  ...
  const cacheTokenThreshold =
    cacheControl?.enabled === false
      ? 0
      : (cacheControl?.tokenThreshold ?? ANTHROPIC_CACHE_DEFAULT_TOKEN_THRESHOLD)
  const cacheLastNMessages =
    cacheControl?.cacheLastNMessages ?? ANTHROPIC_CACHE_DEFAULT_LAST_N_MESSAGES
  ...
  const effectiveCacheTokenThreshold = clampInteger(
    tokenThresholdDraft,
    0,
    CACHE_TOKEN_THRESHOLD_MAX
  )
  ...
  if (!provider) {
    return <ProviderSettingsDrawer open={open} onClose={onClose} title={'API 设置'} />
  }
              ...
              <PageSidePanelItem
                title={
                  <OptionTitle
                    ...
                    label={'缓存 Token 阈值'}
                    help={'消息超过此 Token 数才会被缓存，设为 0 禁用缓存'}
                  />
                }
                action={
                  <Input
