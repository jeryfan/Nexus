...
function toApiKeyEntries(value: string, apiKeysData: ApiKeysData | undefined): ApiKeyEntry[] {
  ...
  for (const key of nextEnabledKeys) {
    const matchedEntry = existingKeys.find(
      (item) => !usedEntryIds.has(item.id) && item.key.trim() === key
    )
    ...
    while (
      enabledCursor < existingEnabledKeys.length &&
      usedEntryIds.has(existingEnabledKeys[enabledCursor].id)
    ) {
      enabledCursor += 1
    }
  ...
  const untouchedDisabledEntries = existingKeys.filter(
    (item) => !item.isEnabled && !usedEntryIds.has(item.id)
  )
...
export function useProviderApiKey(providerId: string) {
  ...
  useEffect(() => {
    ...
    setValue((previousValue) =>
      isSameApiKeyValue(previousValue, nextValue) ? previousValue : nextValue
    )
