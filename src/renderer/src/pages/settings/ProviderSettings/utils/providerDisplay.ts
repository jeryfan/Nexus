...
export function isProviderSettingsListVisibleProvider(_provider: Provider): boolean {
  return true
}

/**
 * Canonical presets that can safely create an independent, URL-based instance
 * through the generic duplicate form. Global-token, local-server, and
 * cloud-account presets need their provider-specific setup instead.
 */
export function isProviderPresetInstanceSource(provider: Provider): boolean {
  ...
  return (
    ...
    provider.endpointConfigs?.[ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS] != null
  )
}
...
export function getFancyProviderName(provider: Provider): string {
  if (isCanonicalPresetProvider(provider)) {
    ...
    if (presetProviderId) {
      return getProviderLabelKey(presetProviderId)
    }
  }
...
export function getProviderSearchString(provider: Provider): string {
  if (isCanonicalPresetProvider(provider)) {
    const presetProviderId = provider.presetProviderId
    if (presetProviderId) {
      return `${getProviderLabelKey(presetProviderId)} ${provider.id}`
    }
  }
 ...
 * Used by DMXAPI domain switching.
 */
