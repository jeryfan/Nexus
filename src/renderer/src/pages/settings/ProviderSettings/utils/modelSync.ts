...
type ModelSyncProviderEndpointSource = Pick<
  Provider,
 ...
 */
async function enrichFetchedModels(
  providerId: string,
  fetchedModels: Partial<Model>[]
): Promise<Model[]> {
  ...
  return filteredModels.map((fetched) => {
    ...
    const registry =
      ...
      resolvedMap.get(
        (apiId.includes('/') ? apiId.substring(apiId.lastIndexOf('/') + 1) : apiId).replaceAll(
          '.',
          '-'
        )
      )
...
export async function fetchProviderCatalogModels(providerId: string): Promise<Model[]> {
  ...
  const preset = (await dataApiService.get(presetPath, {
    query: { fields: 'models' }
  })) as ProviderPreset
