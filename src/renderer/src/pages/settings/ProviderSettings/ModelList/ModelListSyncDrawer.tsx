import { Badge, Button, Input, Tooltip } from '@nexus/ui'
...
export default function ModelListSyncDrawer({
  ...
  const filteredModels = useMemo(() => {
    if (actualFilter === 'stale') {
      return applyModelFilters(allModels, searchText, 'all').filter((model) =>
        staleModelIdSet.has(model.id)
      )
    }
  ...
  const drawerTitle = provider?.name ? `${provider.name} ${'模型'}` : '模型管理'
  const bulkActionLabel = isAllFilteredInProvider ? '移除全部模型' : '添加全部模型'
  const cleanStaleLabel = '清理失效模型'
          ...
          {hasLoadError ? (
            <Tooltip content={loadErrorMessage} placement="top">
              <Button
                ...
                variant="ghost"
                size="icon-sm"
                aria-label={loadErrorMessage ?? '刷新'}
                disabled={isLoading}
                loading={isLoading}
