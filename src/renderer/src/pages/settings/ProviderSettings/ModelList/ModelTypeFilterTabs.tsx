import { Tabs, TabsList, TabsTrigger } from '@nexus/ui'
...
const CAPABILITY_FILTER_LABELS: Record<ModelTypeFilter, string> = {
  text: '文本',
  image: '图片',
  embedding: '嵌入',
  audio: '音频',
      ...
      <TabsList className={cn(modelSyncClasses.manageTabsList, listClassName)}>
        ...
        {extraTabs.map((tab) => (
          ...
          >
            ...
            <span
              className={cn(modelSyncClasses.manageTabCount, tab.destructive && 'text-error-text')}
              aria-hidden
            >
              {tab.count}
            </span>
        ...
        {MODEL_TYPE_FILTERS.map((filter) => {
          ...
          return (
            <TabsTrigger key={filter} value={filter} className={modelSyncClasses.manageTabsTrigger}>
              ...
              <span className="truncate">{CAPABILITY_FILTER_LABELS[filter]}</span>
              <span className={modelSyncClasses.manageTabCount} aria-hidden>
                {counts[filter]}
              </span>
            </TabsTrigger>
