...
export const isValidNewApiModel = (model: Model): boolean =>
  !!(model.endpointTypes && model.endpointTypes.length > 0)

export const filterProviderSettingModelsByKeywords = <T extends Model>(
  keywords: string,
  models: T[]
): T[] => {
...
export const getDuplicateProviderSettingModelNames = <T extends Pick<Model, 'name'>>(
  models: T[]
): Set<string> => {
