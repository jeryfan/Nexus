  ...
  const addSingleModel = useCallback(
    async (values: ModelBasicFormState) => {
      ...
      if (models.some((model) => model.id.endsWith(`::${modelId}`))) {
        setSubmitError('模型已存在')
        return false
      }

      const classifiedCapabilities = buildModelCapabilities(
        prefill?.model?.capabilities ?? [],
        classification
      )
      const classifiedInputModalities = buildModelInputModalities(
        prefill?.model?.inputModalities ?? [],
        classification
      )
      ...
      <ProviderSection className={drawerClasses.section}>
        <div className={drawerClasses.fieldList}>
          <ModelBasicFields
            ...
            modelIdError={modelIdTouched && !formState.modelId.trim() ? '请输入模型 ID' : undefined}
            endpointTypeError={endpointTypeTouched ? '请选择端点类型' : undefined}
            ...
            <div className={drawerClasses.sectionCard}>
              <ModelContextWindowFields
                ...
                onContextWindowChange={(value) =>
                  setFormState((current) => ({ ...current, contextWindow: value }))
                }
                onMaxInputTokensChange={(value) =>
                  setFormState((current) => ({ ...current, maxInputTokens: value }))
                }
