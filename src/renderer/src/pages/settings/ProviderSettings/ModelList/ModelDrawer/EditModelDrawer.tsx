...
const symbolToCurrency = (symbol: string): ModelDrawerCurrency | undefined =>
...
export default function EditModelDrawer({
  providerId,
  open,
  model: modelProp,
  onClose
  ...
  const [classification, setClassification] = useState<ModelClassificationState>(() =>
    getInitialModelClassification()
  )
  ...
  const hasClassificationChanges = !areModelClassificationsEqual(
    classification,
    savedClassification
  )
  ...
  const buildPatch = useCallback(
    (overrides?: BuildPatchOverrides): Partial<Model> => {
      ...
      const hasEndpointTypesOverride =
        overrides != null && Object.hasOwn(overrides, 'endpointTypes')
      const hasPurposeFieldsOverride =
        overrides != null && Object.hasOwn(overrides, 'purposeFields')
      const nextPurposeFields = overrides?.purposeFields ?? purposeFields
      const nextClassification = overrides?.classification
      const shouldApplyPurpose =
        mode === 'purpose' && (hasPurposeFieldsOverride || nextClassification != null)
      ...
      return {
        ...
        ...(hasPurposeFieldsOverride && resolvedPurposeFields
          ...
          : hasEndpointTypesOverride
            ? {
                endpointTypes:
                  mode === 'endpoint-types' ? [...(overrides.endpointTypes ?? [])] : undefined
              }
            : {}),
  ...
  if (!provider || !model) {
    return <ProviderSettingsDrawer open={open} onClose={onClose} title={'编辑模型'} />
  }

  const currentCurrency = currencySymbol || '$'

  return (
    <ProviderSettingsDrawer open={open} onClose={onClose} title={'编辑模型'}>
      <form
        ...
        {showMoreSettings && (
          <ProviderSection className={drawerClasses.section}>
            <div data-testid="provider-settings-model-more-settings" className="space-y-4">
              ...
              <div className={drawerClasses.switchCard}>
                <div className="flex min-w-0 items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    <Tooltip
                      content={
                        '模型每次返回文本增量，而不是一次性返回所有文本，默认开启，如果模型不支持，请关闭'
                      }
                    >
              ...
              <div className={drawerClasses.sectionCard}>
                <ProviderField title={'币种'} titleClassName={drawerClasses.fieldTitle}>
                  <div className={drawerClasses.inlineRow}>
                    ...
                    >
                      <SelectTrigger aria-label={'币种'} className={drawerClasses.selectTrigger}>
                        <SelectValue />
                      </SelectTrigger>
                ...
                <ProviderField title={'输入价格'} titleClassName={drawerClasses.fieldTitle}>
                  <div className={drawerClasses.responsiveValueRow}>
                    ...
                    <span className={drawerClasses.valueSuffix}>
                      {currentCurrency} / {'百万 Token'}
                    </span>
                  </div>
                </ProviderField>

                <ProviderField title={'输出价格'} titleClassName={drawerClasses.fieldTitle}>
                  <div className={drawerClasses.responsiveValueRow}>
                    <Input
                      type="number"
                      min="0"
