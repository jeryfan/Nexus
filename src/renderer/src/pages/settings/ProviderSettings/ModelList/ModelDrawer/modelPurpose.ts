...
function isModelChatEndpointType(
  endpointType: string | undefined
): endpointType is ModelChatEndpointType {
...
export function getModelDrawerMode(provider: ModelDrawerProvider): ModelDrawerMode {
  if (
    matchesPreset(provider, 'new-api') ||
    (provider.presetProviderId == null && !isSystemProviderId(provider.id))
  ) {
    return 'purpose'
  }
  return 'legacy'
...
export function applyModelPurpose(
  ...
  if (previousPurpose !== 'chat') {
    capabilities = capabilities.filter(
      (capability) => capability !== MODEL_CAPABILITY.IMAGE_GENERATION
    )
  ...
  if (purpose === 'chat') {
    return {
      endpointTypes: [
        options.chatEndpointType ??
          getInitialChatEndpointType(fields, ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS)
      ],
      capabilities,
      inputModalities,
      outputModalities
