...
export const MODEL_ENDPOINT_OPTIONS = [
  ...
  { id: ENDPOINT_TYPE.OPENAI_RESPONSES, label: 'OpenAI-Response' },
  { id: ENDPOINT_TYPE.ANTHROPIC_MESSAGES, label: 'Anthropic' },
  { id: ENDPOINT_TYPE.GOOGLE_GENERATE_CONTENT, label: 'Gemini' },
  { id: ENDPOINT_TYPE.OPENAI_EMBEDDINGS, label: '嵌入 (OpenAI)' },
  { id: ENDPOINT_TYPE.OPENAI_IMAGE_GENERATION, label: '图像生成 (OpenAI)' },
  { id: ENDPOINT_TYPE.OPENAI_IMAGE_EDIT, label: '图像编辑 (OpenAI)' },
  { id: ENDPOINT_TYPE.JINA_RERANK, label: 'Jina 重排序' }
] as const
...
export function getInitialAddModelFormState(
  ...
  return {
    ...
    maxInputTokens:
      prefill?.model?.maxInputTokens != null ? String(prefill.model.maxInputTokens) : '',
    maxOutputTokens:
      prefill?.model?.maxOutputTokens != null ? String(prefill.model.maxOutputTokens) : '',
...
export function getInitialModelClassification(model?: Model | null): ModelClassificationState {
  ...
  if (capabilities.includes(MODEL_CAPABILITY.RERANK)) {
    ...
    capabilities.some((capability) => UNEDITABLE_MODEL_TYPE_CAPABILITIES.has(capability))
  ) {
...
export function areModelClassificationsEqual(
  left: ModelClassificationState,
  right: ModelClassificationState
): boolean {
