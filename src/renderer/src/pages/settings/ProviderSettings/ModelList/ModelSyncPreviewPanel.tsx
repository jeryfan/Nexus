import { Avatar, AvatarFallback, Badge, Button, EmptyState, Spinner, Tooltip } from '@nexus/ui'
...
const ManageModelRow = memo(function ManageModelRow({
  ...
  const actionTooltip = isAdded ? '删除模型' : '添加'
          ...
          {model.description ? (
            <Tooltip content={model.description} placement="top">
              <span
                ...
                className={modelSyncClasses.manageRowDescriptionHelp}
              >
...
export default function ModelSyncPreviewPanel({
  ...
  const entries = useMemo(
    () => Object.entries(modelGroups).filter(([, models]) => models.length > 0),
    [modelGroups]
  )
  ...
  }, [collapsedGroups, entries, searchActive])
  ...
  const renderGroupAction = useCallback(
    (models: Model[]) => {
      ...
      const removableGroupModelIds = models
        ...
        .map((model) => model.id)
      const title = isAllInProvider ? '删除分组' : '添加整个分组'
    ...
    [isApplying, localModelIds, onAddModels, onRemoveModels, removableModelIds]
  )
            ...
            <div className={modelSyncClasses.manageVirtualGroupRow}>
              ...
              >
                ...
                >
                  <ChevronRight
                    className={cn(
                      modelSyncClasses.manageGroupChevron,
