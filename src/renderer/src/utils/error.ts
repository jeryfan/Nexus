...
/**
 * OpenAI-style server error envelope returned by most provider APIs.
 * Parsed opportunistically in `formatErrorMessage` so raw API failures render
 * as `错误: <code> <message>` instead of `[object Object]`.
 */
const ApiServerErrorSchema = z.object({
  error: z.object({
    message: z.string(),
    type: z.string(),
    code: z.string()
...
type ApiServerError = z.infer<typeof ApiServerErrorSchema>
...
export function formatErrorMessage(error: unknown): string {
  ...
  const parseResult = ApiServerErrorSchema.safeParse(error)
  if (parseResult.success) {
    return formatApiServerError(parseResult.data)
  }
...
const serializeInvalidToolInputError = (
...
export const serializeError = (error: AiSdkErrorUnion): SerializedError => {
  ...
  if ('requestBodyValues' in error)
    serializedError.requestBodyValues = safeSerialize(error.requestBodyValues)
  ...
  if ('originalMessage' in error)
    serializedError.originalMessage = safeSerialize(error.originalMessage)
...
export function formatAiSdkError(error: SerializedAiSdkError): string {
  ...
  if (isSerializedAiSdkApiCallError(error)) {
    ...
    if (error.responseHeaders) {
      text += `${'响应首部'}: ${JSON.stringify(error.responseHeaders, null, 2)}\n`
    }
    if (error.responseBody) {
      text += `${'响应内容'}: ${error.responseBody}\n`
    }
    if (error.data) {
...
export const formatApiServerError = (error: ApiServerError) =>
  `${'错误'}: ${error.error.code} ${error.error.message}`
