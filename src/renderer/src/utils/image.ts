...
export function checkEntityImageSize(file: File): string | null {
  return file.size > MAX_ENTITY_IMAGE_UPLOAD_BYTES ? `图片过大（上限 ${'10MB'}）` : null
}
  ...
  if (el) {
    ...
    try {
      ...
      if (totalHeight > MAX_ALLOWED_DIMENSION || totalWidth > MAX_ALLOWED_DIMENSION) {
        ...
        return Promise.reject(new Error('内容尺寸过大'))
      }
...
export async function captureScrollableIframe(
  ...
  const createFontStyle = (css: string): HTMLStyleElement => {
    const style = doc.createElement('style')
    style.setAttribute('data-nexus-inline-fonts', 'true')
...
export const captureScrollableIframeAsDataUrl = async (
  iframeRef: React.RefObject<HTMLIFrameElement | null>
) => {
 ...
 */
export const imageInputToPreviewUrl = async (
  input: ImageInput,
  options: ImagePreviewOptions = {}
): Promise<string> => {
  if (input instanceof SVGElement) {
    const blob =
      options.format === 'svg' ? svgToSvgBlob(input) : await svgToPngBlob(input, options.scale || 3)
