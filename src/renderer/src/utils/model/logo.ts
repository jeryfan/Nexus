import type { IconRef } from '@nexus/ui/icons'
import { resolveIconRef, resolveModelIconRef } from '@nexus/ui/icons'
 ...
 * and `:free`/`:cloud` suffix). The `@nexus/ui` registry can't import
 ...
 */
export function getModelLogoRef(
  model: LogoModel | undefined | null,
  providerId?: string
): IconRef | undefined {
  if (!model) return undefined
  const rawId =
    model.apiModelId ??
    (isUniqueModelId(model.id) ? parseUniqueModelId(model.id).modelId : model.id)
