 ...
 * {@link useResolvedCommandMenu}. Renderer concerns (`hasHandler`) are injected
...
export const resolveCommandDisplayState = (
  ...
  const { definition, preference, context, hasHandler } = options

  return {
    label: definition ? definition.title : command,
    enabled: Boolean(
      definition && hasHandler(command) && evaluateContextExpr(definition.enablement, context)
    ),
