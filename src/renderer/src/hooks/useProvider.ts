...
export function useProviderPreset(
 ...
 * hardcoded Chinese label; custom providers use their user-set name. Returns
 * empty string when the provider is missing.
 */
export function getProviderDisplayName(provider: Provider | undefined): string {
  if (!provider) return ''
  return isSystemProviderId(provider.id) ? getProviderLabelKey(provider.id) : provider.name
}
