...
import type { BrowserViewportPresetId } from '@shared/browser/types'
import {
...
import { openUrl } from '../lib/browser-host'
...
export function BrowserToolbarMenu({
  ...
  const switchBrowserTabProfile = useBrowserStore((s) => s.switchBrowserTabProfile)
  const createBrowserSessionProfile = useBrowserStore((s) => s.createBrowserSessionProfile)
  const importCookiesFromBrowser = useBrowserStore((s) => s.importCookiesFromBrowser)
  const importCookiesToProfile = useBrowserStore((s) => s.importCookiesToProfile)
  const fetchDetectedBrowsers = useBrowserStore((s) => s.fetchDetectedBrowsers)
  const browserSessionImportState = useBrowserStore((s) => s.browserSessionImportState)
  const setBrowserPageViewportPreset = useBrowserStore((s) => s.setBrowserPageViewportPreset)
  // 裁剪: contextual tour 状态读取（activeContextualTourId /
  // activeContextualTourStepIndex）——Nexus 无引导教程体系，恒无活跃 tour。
  const browserCookieTourStepActive = false
  const browserImportHintHidden = useBrowserStore((s) => s.browserImportHintHidden)
  const persistedUIReady = useBrowserStore((s) => s.persistedUIReady)
  ...
  // Glue for Nexus: 原工具栏独立的「在默认浏览器中打开」按钮移入本菜单。
  const handleOpenInDefaultBrowser = (): void => {
    if (externalUrl) {
  ...
  return (
    <>
      <BrowserToolbarMenuDropdown
        ...
        onOpenDevTools={() => void window.api.browser.openDevTools({ browserPageId })}
        canOpenInDefaultBrowser={Boolean(externalUrl)}
        onOpenInDefaultBrowser={handleOpenInDefaultBrowser}
      />
