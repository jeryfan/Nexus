...
import type { BrowserLoadError } from '@shared/browser/types'
import { isChromiumCertificateErrorCode } from '@shared/browser/browser-certificate-errors'
...
export function formatPermissionNotice(event: BrowserPermissionDeniedEvent): string {
  ...
  return `${target} asked for ${humanizePermission(event.permission)}, and Nexus denied it.`
}

export function formatPopupNotice(event: BrowserPopupEvent): string {
  const target = event.origin === 'unknown' ? 'A site' : event.origin
  if (event.action === 'opened-in-nexus') {
    return `${target} opened a new page in Nexus.`
  }
  if (event.action === 'opened-external') {
    return `${target} opened a new window in your default browser.`
  }
  return `${target} tried to open a popup Nexus does not support here.`
}
