...
import type { BrowserHistoryEntry } from '@shared/browser/types'
...
vi.mock('@renderer/stores/browser', () => ({
  useBrowserStore: (selector: (state: typeof mocks) => unknown) => selector(mocks)
}))
...
describe('BrowserAddressBar autocomplete preview', () => {
  ...
  it('restores the typed query when a previewed suggestion is dismissed by blur', async () => {
    ...
    const input = container.querySelector<HTMLInputElement>('input[data-nexus-browser-address-bar]')
