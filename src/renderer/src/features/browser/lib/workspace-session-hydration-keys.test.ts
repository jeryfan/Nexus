...
describe('collectWorktreeHydrationRepoIdsFromSession', () => {
  ...
  // 裁剪: sleeping-agent 分支随 agent-session-resume 特性裁剪，
  // 用例去掉 sleepingAgentSessionsByPaneKey fixture 与 'repo-agent' 期望。
  it('includes repositories referenced only by active, editor, or browser state', () => {
    const session = {
      activeRepoId: null,
