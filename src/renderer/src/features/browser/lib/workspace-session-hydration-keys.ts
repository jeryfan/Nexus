// 裁剪: sleepingAgentSessionsByPaneKey 扫描分支随 agent-session-resume
// 特性在 Nexus 中被裁剪（见 @shared/browser/types.ts 头部裁剪清单），其余逻辑原样保留。
import type { ExecutionHostId } from '@shared/browser/execution-host'
import { parseExecutionHostId } from '@shared/browser/execution-host'
import type { WorkspaceKey, WorkspaceSessionState } from '@shared/browser/types'
import { parseWorkspaceKey } from '@shared/browser/workspace-scope'
import { getRepoIdFromWorktreeId } from '@shared/browser/worktree-id'
