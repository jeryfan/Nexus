...
// Glue for Nexus: 由 createBrowserSlice: StateCreator<AppState, [], [], BrowserSlice> 改造为独立
// zustand store（useBrowserStore），状态字段与 action 逻辑保持原样；跨 slice 依赖按以下方式接入：
//   - 统一 tab 系统（unifiedTabsByWorktree/createUnifiedTab/closeUnifiedTab/activateTab/setTabLabel）
//     → features/browser/panel-bridge（PanelTab.id === BrowserWorkspace.id）
//   - recordFeatureInteraction → panelBridge.recordFeatureInteraction（no-op，Nexus 无埋点设施）
//   - settings.browserDefaultUrl → BROWSER_SETTINGS_DEFAULTS（features/browser/settings-defaults）
//   - 原 AppState 跨 slice 字段（activeWorktreeId/activeTabType/tabBarOrderByWorktree/
//     tabsByWorktree/openFiles/recentlyClosedTabKindsByWorktree）→ 本 store 自有（见 BrowserState）
// 裁剪: remote runtime 分支（@/runtime/*、worktree-runtime-owner、callRuntimeRpc、
// createWebRuntimeSessionBrowserTab）、buildValidWorktreeIdsForSessionHydration 过滤、folderWorkspaces。
...
import { GRAB_BUDGET, type BrowserPageAnnotation } from '@shared/browser/browser-grab-types'
import { FLOATING_TERMINAL_WORKTREE_ID, NEXUS_BROWSER_BLANK_URL } from '@shared/browser/constants'
import { redactKagiSessionToken } from '@shared/browser/browser-url'
import {
  MAX_BROWSER_HISTORY_ENTRIES,
  normalizeBrowserHistoryEntries,
  normalizeBrowserHistoryUrl
} from '@shared/browser/workspace-session-browser-history'
// 裁剪: shared/runtime-types 的 Browser*Result 类型仅用于远程 runtime RPC 分支，随分支裁剪。
// 裁剪: getSettingsFocusedExecutionHostId(state.settings) —— Nexus 无 GlobalSettings/
// 远程 runtime，设置焦点 host 固定为本地（见 getBrowserSettingsHostId）。
import {
  LOCAL_EXECUTION_HOST_ID,
  parseExecutionHostId,
  toRuntimeExecutionHostId,
  type ExecutionHostId
} from '@shared/browser/execution-host'
import { createBrowserUuid } from '../features/browser/lib/browser-uuid'
import { translate } from '../features/browser/i18n'
import { pickNeighbor } from '../features/browser/lib/pick-neighbor'
import {
  getRecentlyClosedTabPosition,
  restoreRecentlyClosedTabPosition,
  pushRecentlyClosedTabKind,
  type RecentlyClosedTabKind,
  type RecentlyClosedTabPosition
} from '../features/browser/lib/recently-closed-tabs'
import {
...
} from '../features/browser/settings-defaults'
import type { KeybindingOverrides } from '@shared/browser/keybindings'
...
function getBrowserSettingsHostId(
  state: Pick<BrowserState, 'browserSessionHostIdOverride'>
): ExecutionHostId {
  // 裁剪: getSettingsFocusedExecutionHostId(state.settings) —— Nexus 无 GlobalSettings/
  // 远程 runtime，设置焦点 host 固定为本地。
  return state.browserSessionHostIdOverride ?? LOCAL_EXECUTION_HOST_ID
}
...
function getBrowserWorktreeHostId(_state: BrowserState, _worktreeId: string): ExecutionHostId {
  // 裁剪: getExecutionHostIdForWorktree（worktree-runtime-owner 远程归属判定）——
  // Nexus 全部为本地宿主。
...
function profileListByHostUpdate(
  state: Pick<BrowserState, 'browserSessionHostIdOverride' | 'browserSessionProfilesByHostId'>,
...
function getDefaultBrowserProfileForHost(
  state: BrowserState,
  hostId: ExecutionHostId
): string | null {
  return (
...
function closeRemoteBrowserPageInOwningEnvironment(
  _worktreeId: string,
  _handle: RemoteBrowserPageHandle
): void {
  // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.tabClose' + toRuntimeWorktreeSelector）——
  // Nexus 无远程 runtime，远程 page handle 不会产生。
}
...
function buildBrowserPage(
  ...
  return {
    ...
    loading: normalizedUrl !== 'about:blank' && normalizedUrl !== NEXUS_BROWSER_BLANK_URL,
    faviconUrl: null,
    canGoBack: false,
    canGoForward: false,
    loadError: null,
...
function getFallbackTabTypeForWorktree(
  worktreeId: string,
  openFiles: BrowserState['openFiles'],
  terminalTabsByWorktree: BrowserState['tabsByWorktree'],
  browserTabsByWorktree?: BrowserState['browserTabsByWorktree']
): WorkspaceVisibleTabType {
...
// 原经 AppState 读写的跨 slice 字段，在独立 store 中由本 store 自有。
export type BrowserState = BrowserSlice & {
  /** 当前活跃会话 id（worktreeId→sessionId 映射）。
   *  由会话切换胶水（Task 10 useBrowserSessionHydration）维护。 */
  activeWorktreeId: string | null
  setActiveWorktreeId: (worktreeId: string | null) => void
  /** 活跃标签类型（Nexus 无终端/编辑器 slice，仅浏览器逻辑内部使用） */
  activeTabType: WorkspaceVisibleTabType
  activeTabTypeByWorktree: Record<string, WorkspaceVisibleTabType>
  /** 混合标签栏顺序（closeBrowserTab 的邻居选择依赖它） */
  tabBarOrderByWorktree: Record<string, string[]>
  setTabBarOrder: (worktreeId: string, order: string[]) => void
  /** 终端标签桩：Nexus 浏览器 store 不管理终端标签，仅供 tab 栏排序/回退类型计算 */
  tabsByWorktree: Record<string, Array<{ id: string }>>
  /** 编辑器文件桩：同上 */
  openFiles: Array<{ id: string; worktreeId: string }>
  /** 跨类型关闭种类账本（Cmd+Shift+T 重开顺序） */
  recentlyClosedTabKindsByWorktree: Record<string, RecentlyClosedTabKind[]>
  /** hydrate 成功后才允许落盘，
   *  避免以空状态覆盖磁盘会话文件 */
  browserSessionReady: boolean
  /** 用户自定义快捷键 overrides：Nexus 无快捷键自定义，恒空 */
  keybindings: KeybindingOverrides
  /** 浏览器默认缩放级别 → BROWSER_SETTINGS_DEFAULTS（会话内可变） */
  browserDefaultZoomLevel: number | null
  setBrowserDefaultZoomLevel: (level: number | null) => void
  /** 浏览器默认搜索引擎 / Kagi 会话链接 → BROWSER_SETTINGS_DEFAULTS */
  browserDefaultSearchEngine: BrowserSearchEngine | null
  browserKagiSessionLink: string | null
  /** 功能使用埋点：Nexus 无遥测设施，no-op */
  recordFeatureInteraction: (feature: string) => void
  /** 账号 profile 体系未引入，恒 null */
  activeAppProfileId: string | null
  /** 导入提示隐藏 / 持久化 UI 就绪标记：Nexus 无持久化 UI 快照，hint 仅会话内记忆 */
  browserImportHintHidden: boolean
  setBrowserImportHintHidden: (hidden: boolean) => void
  persistedUIReady: boolean
  /** 跳设置页：Nexus 经 navigation store 落地 */
...
export const useBrowserStore = create<BrowserState>()((set, get) => ({
  ...
  // Glue for Nexus: 跨 slice 字段初始值（见 BrowserState 注释）。
  ...
  // 面板组件读取的跨 slice 设置/UI 字段（见 BrowserState 注释）。
  keybindings: {},
  browserDefaultZoomLevel: BROWSER_SETTINGS_DEFAULTS.browserDefaultZoomLevel,
  browserDefaultSearchEngine: BROWSER_SETTINGS_DEFAULTS.browserDefaultSearchEngine,
  browserKagiSessionLink: BROWSER_SETTINGS_DEFAULTS.browserKagiSessionLink,
  ...
  recordFeatureInteraction: () => {
    // 遥测埋点：Nexus 无遥测设施，no-op。
  },
  openSettingsPage: () => {
    // 跳转设置页：经 navigation store。
    useNavigationStore.getState().navigate('settings')
  },
  openSettingsTarget: () => {
    // 设置页分区定位（pane/repoId）；浏览器设置分区已落地
    // （features/browser/settings），但设置页尚不支持指定分区锚点，仅跳设置页。
    useNavigationStore.getState().navigate('settings')
  },
  setTabBarOrder: (worktreeId, order) =>
    set((s) => ({
      tabBarOrderByWorktree: { ...s.tabBarOrderByWorktree, [worktreeId]: order }
    })),
  ...
  createBrowserTab: (worktreeId, url, options) => {
    ...
    set((s) => {
      ...
      const shouldFocusAddressBar =
        ...
        (options?.focusAddressBar ??
          (page.url === 'about:blank' || page.url === NEXUS_BROWSER_BLANK_URL))
    ...
    // 面板标签联动：经 panelBridge.openBrowserTab 打开
    // （PanelTab.id === BrowserWorkspace.id）。裁剪: targetGroupId/activate 参数
    // （Nexus 面板无分组概念，openTab 总是激活；原各调用点 activate 均为 true）。
    if (!panelBridge.hasTab(workspaceId)) {
      panelBridge.openBrowserTab(workspaceId, browserTab.title)
    }
    return browserTab
  },
  ...
  openNewBrowserTabInActiveWorkspace: async (groupId) => {
    ...
    // 裁剪: remote runtime 分支（getRuntimeEnvironmentIdForWorktree +
    ...
    // Glue for Nexus: recordFeatureInteraction → panelBridge（no-op）。
    panelBridge.recordFeatureInteraction('browser-tab-created')
  },
  closeBrowserTab: (tabId) => {
    ...
    // 面板标签联动：经 panelBridge.closeTab 关闭
    // （PanelTab.id === BrowserWorkspace.id，内部自带存在性检查）。
    panelBridge.closeTab(tabId)
  },
  ...
  setActiveBrowserTab: (tabId) => {
    ...
    // 面板标签联动：经 panelBridge.activateTab 激活（PanelTab.id === workspace id）。
    panelBridge.activateTab(tabId)
  },
  ...
  createBrowserPage: (workspaceId, url, options) => {
    ...
    if (nextWorkspace?.activePageId === page.id) {
      // 面板标签标题同步：panelBridge.setTabLabel（无对应面板标签时 no-op）。
      panelBridge.setTabLabel(workspaceId, page.title)
    }
    return page
  },
  ...
  closeBrowserPage: (pageId) => {
    ...
    if (workspace) {
      // 面板标签标题同步：panelBridge.setTabLabel。
      panelBridge.setTabLabel(closedWorkspaceId, workspace.title)
    }
  },
  ...
  setActiveBrowserPage: (workspaceId, pageId) => {
    ...
    // 面板标签标题同步：panelBridge.setTabLabel。
    panelBridge.setTabLabel(workspaceId, workspace.title)
  },
  ...
  focusBrowserTabInWorktree: (worktreeId, browserPageId, options) => {
    ...
    // 面板标签联动：经 panelBridge.activateTab 激活（PanelTab.id === workspace id）。
    panelBridge.activateTab(workspace.id)
  },
  ...
  updateBrowserPageState: (pageId, updates) => {
    // 标签 label 存于 projectPanel store（而非本 store 的 set() 内），故在 set() 前
    // 计算 label 修复条件，set() 后经 panelBridge 执行 rename。
    const prePage = findPage(get().browserPagesByWorkspace, pageId)
    const preWorkspace = prePage
      ? findWorkspace(get().browserTabsByWorktree, prePage.workspaceId)
      : null
    const preNextTitle = prePage
      ? normalizeBrowserTitle(
          updates.title === undefined ? prePage.title : updates.title,
          prePage.url
        )
      : null
    const prePanelTab = preWorkspace
      ? useProjectPanelStore.getState().tabs.find((t) => t.id === preWorkspace.id)
      : undefined
    const unifiedLabelNeedsRepair =
      preWorkspace !== null &&
      preWorkspace.activePageId === pageId &&
      updates.title !== undefined &&
      prePanelTab !== undefined &&
      prePanelTab.label !== preNextTitle
    ...
    set((s) => {
      ...
      if (pageStateUnchanged) {
        const nextState: Partial<BrowserState> = {}
    ...
    if (unifiedLabelNeedsRepair && preWorkspace && preNextTitle !== null) {
      panelBridge.setTabLabel(preWorkspace.id, preNextTitle)
    }
  ...
  setBrowserPageUrl: (pageId, url) => {
    ...
    if (nextUrl !== 'about:blank' && nextUrl !== NEXUS_BROWSER_BLANK_URL) {
      const currentPage = findPage(get().browserPagesByWorkspace, pageId)
      if (currentPage) {
        // Glue for Nexus: recordFeatureInteraction → panelBridge（no-op）。
        panelBridge.recordFeatureInteraction('browser')
      }
    }
    set((s) => {
      const page = findPage(s.browserPagesByWorkspace, pageId)
  ...
  hydrateBrowserSession: (session, options) => {
    ...
    // 裁剪: buildValidWorktreeIdsForSessionHydration 过滤——Nexus 无 repo/worktree
    // 目录概念，不过滤历史 worktreeId（即会话 id），持久化里的所有 key 都视为有效；
    // folderWorkspaces 循环随之裁剪（Nexus 无 folder workspace 概念）。
    const validWorktreeIdsForCleanup = new Set(Object.keys(persistedTabsByWorktree))
    ...
    set((s) => {
      ...
      // 裁剪: 同上（buildValidWorktreeIdsForSessionHydration / folderWorkspaces）。
      const validWorktreeIds = new Set(Object.keys(persistedTabsByWorktree))
      ...
      for (const worktreeId of validWorktreeIds) {
        ...
        // Note (Nexus): activeTabTypeByWorktree 不在持久化的 4 个浏览器字段之列（见
        // session/workspace-session.ts SESSION_RELEVANT_FIELDS），session.activeTabTypeByWorktree
        // 恒为 undefined → persistedActiveTabTypeByWorktree 恒为 {}，此分支在 Nexus 中不会命中。
        // 保留原逻辑不改行为：浏览器活跃类型的恢复由下方空桩 fallback 路径等价覆盖。
        if (
    ...
    const state = get()
    for (const [_worktreeId, browserTabs] of Object.entries(state.browserTabsByWorktree)) {
      for (const bt of browserTabs) {
        // 面板标签联动：经 panelBridge.openBrowserTab 打开；
        // recordInteraction: false 无需特殊处理（panelBridge.recordFeatureInteraction 本身 no-op）。
        if (!panelBridge.hasTab(bt.id)) {
          panelBridge.openBrowserTab(bt.id, bt.title)
        }
      }
    }

    // Task 5 定案：水合后把当前激活浏览器页上报主进程（CLI 无 selector 命令走 bridge 全局活跃 tab）。
    // setActiveBrowserTab/setActiveBrowserPage/focusBrowserTabInWorktree 中也会发送；
    // 水合发送点为 Nexus 新增（原实现依赖布局恢复后的 UI effect）。
    const activeWorkspace = state.activeBrowserTabId
      ? findWorkspace(state.browserTabsByWorktree, state.activeBrowserTabId)
      : null
    const activeHydratedPage = activeWorkspace?.activePageId
    ...
    if (
      ...
      isLocalBrowserPageOwner(
        state,
        activeWorkspace.worktreeId,
        activeHydratedPage?.browserRuntimeEnvironmentId
      ) &&
      typeof window !== 'undefined' &&
      window.api?.browser
    ) {
  ...
  fetchBrowserSessionProfiles: async () => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileList'）——固定走本地 IPC。
  ...
  createBrowserSessionProfile: async (scope, label) => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileCreate'）。
    const hostId = getBrowserSettingsHostId(get())
    try {
  ...
  deleteBrowserSessionProfile: async (profileId) => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileDelete'）。
    const hostId = getBrowserSettingsHostId(get())
    try {
  ...
  importCookiesToProfile: async (profileId) => {
    ...
    try {
      ...
      if (result.ok) {
        panelBridge.recordFeatureInteraction('cookie-import')
        set((state) =>
          browserImportStateForHostUpdate(state, hostId, {
            profileId,
            status: 'success',
  ...
  fetchDetectedBrowsers: async () => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileDetectBrowsers'）。
    const hostId = getBrowserSettingsHostId(get())
  ...
  importCookiesFromBrowser: async (profileId, browserFamily, browserProfile?) => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileImportFromBrowser'）。
    const hostId = getBrowserSettingsHostId(get())
    set((state) =>
      browserImportStateForHostUpdate(state, hostId, {
        profileId,
  ...
  clearDefaultSessionCookies: async () => {
    // 裁剪: remote runtime 分支（callRuntimeRpc 'browser.profileClearDefaultCookies'）。
    const hostId = getBrowserSettingsHostId(get())
    try {
  ...
  addBrowserHistoryEntry: (url, title) => {
    ...
    if (safeUrl === NEXUS_BROWSER_BLANK_URL || safeUrl === 'about:blank' || !safeUrl) {
      return
    }
