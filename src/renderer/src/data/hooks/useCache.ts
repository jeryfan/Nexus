...
import {
  findMatchingSharedCacheSchemaKey,
  isTemplateKey,
  templateToRegex
} from '@shared/data/cache/templateKey'
 ...
 * `internal.shared_cache_text.*` keys fall back to the single default
 * `''`. This mirrors getUseCacheDefaultValue semantics.
 */
function getSharedCacheDefaultValue<K extends SharedCacheKey>(
  key: K
): InferSharedCacheValue<K> | undefined {
 ...
 * const [counter, setCounter] = useCache('internal.renderer_cache_counter', 10)
 *
 * // Update the value
 ...
 * - Fixed keys: `useSharedCache('internal.shared_cache_flag')`
 * - Template keys: `useSharedCache('internal.shared_cache_text.provider-a')`
 *   matches schema entry `'internal.shared_cache_text.${id}'`
 *
...
export function useSharedCache<K extends SharedCacheKey>(
   ...
   * shared default (e.g. all 'internal.shared_cache_text.*' instances
    ...
    (newValue: CacheSetStateAction<InferSharedCacheValue<K>>) => {
      if (typeof newValue === 'function') {
        const prev = (cacheService.getShared(key) ??
          initValue ??
          getSharedCacheDefaultValue(key)) as ReadonlyValue<InferSharedCacheValue<K>>
  ...
  return [
    value ?? initValue ?? (getSharedCacheDefaultValue(key) as InferSharedCacheValue<K>),
    setValue
  ]
}
 ...
 *   return (
 *     useSharedCacheValue(`internal.shared_cache_progress.${jobId}` as const) ?? EMPTY_PROGRESS
 *   )
 * }
 * ```
 */
export function useSharedCacheValue<K extends SharedCacheKey>(
  key: K
): InferSharedCacheValue<K> | undefined {
...
const readSharedKey = (key: string): unknown =>
  cacheService.getSharedSnapshot(key as SharedCacheKey)
