 ...
 * `useQuery(collectionUrl)`. It supports two common cache shapes out of the box:
 *
 * - **Flat array** `T[]` — the active `/providers` shape
 * - **Wrapped collection** `{ items: T[] }` — retained by the generic helper
 *
...
export function useReorder<TCollection extends ConcreteApiPaths>(
  ...
  if (hasSelect !== hasUpdate) {
    throw new Error(
      'useReorder: options.selectItems and options.updateItems must be provided together'
    )
  }
  ...
  const readCurrent = useCallback(
    (): unknown => readCache<unknown>(collectionUrl),
  ...
  const move = useCallback(
    async (id: string, anchor: OrderRequest) => {
      ...
      const optimistic =
        items !== undefined
          ? updateItems(current, computeOptimistic(items, id, anchor, idKey))
          : undefined
      ...
      try {
        ...
        logger.warn(`move failed for ${String(collectionUrl)} id=${id}, rolling back`, {
          error: err
        })
  ...
  const applyBatch = useCallback(
    ...
    async (current: unknown, items: ItemList, moves: OrderBatchRequest['moves']) => {
      ...
      try {
        ...
        logger.warn(`batch reorder failed for ${String(collectionUrl)}, rolling back`, {
          error: err
        })
        await invalidateCache(collectionUrl)
        throw err
  ...
  const applyReorderedList = useCallback(
    async (newList: ItemList) => {
      ...
      if (current === undefined) {
        logger.warn(
          `applyReorderedList called before data loaded at ${String(collectionUrl)}; ignored`
        )
        return
      }
      const items = selectItems(current)
