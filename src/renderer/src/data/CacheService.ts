...
const STORAGE_PERSIST_KEY = 'nexus_cache_persist'
...
export class CacheService {
   ...
   * cacheService.set('internal.renderer_cache_counter', 1, 30000)
   * ```
   */
  ...
  setPersist<K extends RendererPersistCacheKey>(
    ...
    const nextValue =
      typeof value === 'function'
        ? value(this.getPersist(key) as ReadonlyValue<RendererPersistCacheSchema[K]>)
        : value
  ...
  public getStats(includeDetails: boolean = false): CacheStats {
    ...
    // Calculate totals
    const totalBytes =
      memory.summary.estimatedBytes + shared.summary.estimatedBytes + persist.summary.estimatedBytes

    const total = {
      totalCount:
        memory.summary.totalCount + shared.summary.totalCount + persist.summary.totalCount,
      validCount:
        memory.summary.validCount + shared.summary.validCount + persist.summary.validCount,
      ...
      hookReferences:
        memory.summary.hookReferences +
        shared.summary.hookReferences +
        persist.summary.hookReferences,
  ...
  private savePersistCache(): void {
    try {
      ...
      logger.verbose(
        `Saved persist cache to localStorage, size: ${(size / (1024 * 1024)).toFixed(2)} MB`
      )
    } catch (error) {
