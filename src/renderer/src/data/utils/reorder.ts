...
export function computeMinimalMoves<T extends Record<string, unknown>>(
  ...
  for (let i = 0; i < newList.length; i++) {
    ...
    if (idx === undefined) {
      throw new Error(
        `computeMinimalMoves: newList id "${newId}" does not exist in currentList (not a permutation)`
      )
    }
    perm[i] = idx
  }
  ...
  if (new Set(perm).size !== perm.length) {
    throw new Error(
      'computeMinimalMoves: newList is not a permutation of currentList (duplicate ids)'
    )
  }
