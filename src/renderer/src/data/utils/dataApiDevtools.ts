...
declare global {
  interface Window {
    __NEXUS_DATA_API_DEVTOOLS__?: DataApiDevtoolsGlobal
  }
}
...
function installGlobal(): void {
  if (!isEnabled() || window.__NEXUS_DATA_API_DEVTOOLS__) return

  window.__NEXUS_DATA_API_DEVTOOLS__ = {
...
export const dataApiDevtoolsTesting = {
  ...
  reset: () => {
    ...
    if (typeof window !== 'undefined') {
      delete window.__NEXUS_DATA_API_DEVTOOLS__
    }
  }
}
