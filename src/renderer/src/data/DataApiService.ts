...
export class DataApiService implements ApiClient {
  ...
  private dataChangeListeners = new Map<
    GetMethodApiPaths,
    Set<(effects: DataApiDataChangeEffect[]) => void>
  >()
    ...
    try {
      ...
      const response = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(DataApiErrorFactory.timeout(request.path, 3000, requestContext)),
            3000
          )
        )
      ])
      ...
      const apiError =
        error instanceof DataApiError
          ? error
          : toDataApiError(error, `${request.method} ${request.path}`)
      ...
      if (retryCount < this.defaultRetryOptions.maxRetries && apiError.isRetryable) {
        ...
        const delay =
          this.defaultRetryOptions.retryDelay *
          Math.pow(this.defaultRetryOptions.backoffMultiplier, retryCount)
  ...
  private dispatchDataChange(effects: DataApiDataChangeEffect[]): void {
    const matched = new Map<
      (effects: DataApiDataChangeEffect[]) => void,
      DataApiDataChangeEffect[]
    >()
