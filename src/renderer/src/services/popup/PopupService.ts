import { DIALOG_UNMOUNT_DELAY_MS } from '@nexus/ui/utils'
...
class PopupService {
  ...
  subscribe = (listener: () => void): (() => void) => {
    ...
    if (this.listeners.size > 1) {
      logger.error(
        'multiple PopupHost mounted in one window; every popup will render once per host'
      )
    }
    return () => {
  ...
  showComponent<P extends object, R>(
    ...
    if (!this.hasHost()) {
      logger.warn('createPopup show() with no PopupHost mounted; resolving dismissResult', {
        instanceId
      })
