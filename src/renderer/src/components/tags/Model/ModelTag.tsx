...
export type ModelDisplayTagSource = Pick<
  Model,
  'id' | 'name' | 'providerId' | 'capabilities' | 'inputModalities'
>
...
export function modelMatchesDisplayTag(model: ModelDisplayTagSource, tag: ModelDisplayTag) {
  ...
  return (
    model.capabilities.includes(tag) ||
    Boolean(inputModality && model.inputModalities?.includes(inputModality))
  )
}
...
export function getModelDisplayTags(
  ...
  return MODEL_DISPLAY_TAGS.filter(
    (tag) => isModelTagVisible(tag, options) && modelMatchesDisplayTag(model, tag)
  )
}
