import { Divider as SettingDivider } from '@nexus/ui'
...
export const SettingContainer = ({
  ...
  <div
    ...
    className={cn(
      'flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto p-4 [&::-webkit-scrollbar]:hidden',
      className
    )}
    {...props}
  />
...
// Canonical settings page container — mirrors the model service detail column:
// outer p-6 + inner mx-auto max-w-3xl. Pages with an internal split layout use
// SettingContainer instead.
...
export const SettingTitle = ({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) => (
  <div
    className={cn(
      'flex select-none items-center justify-between font-semibold text-[15px]',
      className
    )}
    {...props}
  />
...
export const SettingRowTitle = ({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) => (
  <div
    className={cn(
      'flex min-w-0 flex-wrap items-center text-foreground text-sm leading-4.5',
      className
    )}
    {...props}
  />
...
export const SettingHelpTextRow = ({
  className,
  ...props
}: React.ComponentPropsWithoutRef<'div'>) => (
...
export const SettingHelpLink = ({
  ...
  <a
    className={cn('cursor-pointer text-[11px] text-primary hover:underline', className)}
    {...props}
  />
)
...
export const SettingGroup = ({
  ...
  <div
    ...
    style={{
      backgroundColor:
        variant === 'card' ? 'var(--settings-group-background, var(--card))' : undefined,
      ...style
    }}
    className={cn(
      variant === 'card'
