...
const PopupContainer: React.FC<Props> = ({
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.error({ title: '错误', description: formatErrorMessage(error) })
      setLoading(false)
      return
    }
    resolve(true)
  ...
  const leadingIcon =
    icon === null ? null : (icon ?? <AlertCircle className="mt-0.5 size-5 shrink-0 text-warning" />)
  const okLabel = okText ?? (danger ? '删除' : '确认')
 ...
 */
const ConfirmActionPopup = createPopup<ConfirmActionParams, boolean>(PopupContainer, {
  dismissResult: false
})
