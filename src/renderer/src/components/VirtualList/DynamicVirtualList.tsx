...
function DynamicVirtualList<T>(props: DynamicVirtualListProps<T>) {
  ...
  const internalStickyRangeExtractor = useCallback(
    (range: Range) => {
      ...
      if (getItemDepth) {
        ...
        const lastStickyBeforeRange = [...stickyIndexes]
          .reverse()
  ...
  const rangeExtractor =
    customRangeExtractor ?? (isSticky ? internalStickyRangeExtractor : undefined)
    ...
    >
      ...
      >
        {virtualItems.map((virtualItem) => {
          ...
          const isItemActiveSticky =
            isItemSticky && activeStickyIndexesRef.current.includes(virtualItem.index)

          // Calculate the sticky offset for multi-level sticky headers
          const activeStickyIndex = isItemActiveSticky
            ? activeStickyIndexesRef.current.indexOf(virtualItem.index)
            : -1
          ...
          const activeStickyZIndex =
            ACTIVE_STICKY_ITEM_Z_INDEX_BASE +
            activeStickyIndexesRef.current.length -
            activeStickyIndex
