...
function getEventData<TGroup, TItem>(data: unknown): RowDragData<TGroup, TItem> | null {
  ...
  return rowData.rowType === 'group' || rowData.rowType === 'item'
    ? (rowData as RowDragData<TGroup, TItem>)
    : null
}

function isItemDragData<TGroup, TItem>(
  data: RowDragData<TGroup, TItem>
): data is ItemDragData<TGroup, TItem> {
...
function getItemDropPosition<TGroup, TItem>(
  ...
  const activeCenterY = getRectCenterY(
  ...
  if (
    isItemDragData(active) &&
    active.groupId === over.groupId &&
    active.itemIndexInGroup > over.itemIndexInGroup
  ) {
    return 'before'
  }
...
function getDropPositionFromState<TGroup, TItem>(
  over: RowDragData<TGroup, TItem>,
...
function buildDropPayloadFromStateOrEvent<TGroup, TItem>(
  ...
  const position = appendDropTarget
    ? 'after'
    : (statePosition ?? getDropPosition(event, active, over))
...
function SortableItemRow<TGroup, TItem>({
  ...
  return (
    <div
      ...
      className={joinClassNames(
        dropTargetRowState.props.className,
        dropIndicatorPosition ? 'relative' : undefined
      )}
      style={{
        opacity: isDragging || sourcePlaceholder ? 0.5 : undefined,
        transform:
          dropTargetRowState.isBlocked || freezeTransform
            ? undefined
...
function GroupedSortableVirtualList<TGroup, TItem, THeader = TGroup, TFooter = unknown>(
  ...
  const [activeDragState, setActiveDragState] = useState<ActiveDragState<TGroup, TItem> | null>(
    null
  )
  ...
  const sensors = useSensors(
    useSensor(ContextMenuSafePointerSensor, {
  ...
  const sortableIds = useMemo(
    () =>
      rows.flatMap((row) => {
        if (row.type === 'item') {
          return toItemSortableId(
            getItemId(row.item, row.itemIndex, row.group, row.groupIndex, row.itemIndexInGroup)
          )
        }
  ...
  const estimateRowSize = useCallback(
    (index: number) => {
      ...
      if (row.type === 'group-header') {
        return (
          estimateGroupHeaderSize?.(row.header, row.group, row.groupIndex) ??
          DEFAULT_GROUP_HEADER_SIZE
        )
      }

      if (row.type === 'group-footer') {
        return (
          estimateGroupFooterSize?.(row.footer, row.group, row.groupIndex) ??
          DEFAULT_GROUP_FOOTER_SIZE
        )
      }
      ...
      for (const groupId of groupIds) {
        ...
        const isAllowed = (candidateDataByGroupId.get(groupId) ?? []).some((over) => {
          ...
          if (
            ...
            payload.activeId === payload.overId
          )
            return false
  ...
  const updateOverDropState = useCallback((nextOverDropState: OverDropState) => {
    ...
    setOverDropState((current) =>
      isSameOverDropState(current, nextOverDropState) ? current : nextOverDropState
    )
  }, [])
  ...
  const handleDragOver = useCallback(
    (event: DragOverEvent) => {
      ...
      if (
        !shouldDropPayload(result.payload, effectiveDragCapabilities, canDropGroup, canDropItem)
      ) {
        clearOverDropState()
        return
      }
  ...
  const renderRow = useCallback(
    (row: GroupedSortableVirtualListRow<TGroup, TItem, THeader, TFooter>) => {
      ...
      const itemDisabled =
        ...
        !(
          canDragItem?.(row.item, row.itemIndex, row.group, row.groupIndex, row.itemIndexInGroup) ??
          true
        )
  ...
  const dragOverlayContent = useMemo(() => {
    ...
    const headerRow = rows.find(
      (row) =>
        row.type === 'group-header' && getGroupId(row.group, row.groupIndex) === active.groupId
    )
