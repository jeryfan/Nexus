...
const combineContextMenuItems = (
  ...
  const separator: readonly CommandContextMenuExtraItem[] =
    commandItems.length > 0 && hasNonSeparatorItems(extraItems)
      ? [{ type: 'separator' }]
      : EMPTY_EXTRA_ITEMS

  return removeEmptySeparators<CommandContextMenuItem>([
    ...commandItems,
    ...separator,
    ...extraItems
  ])
...
function CommandMenuItemView({
    ...
    return (
      <ContextMenuSub>
        <ContextMenuSubTrigger disabled={!item.enabled}>
          <ContextMenuItemContent icon={renderIcon?.(item.iconKey)}>
            {item.label}
          </ContextMenuItemContent>
        </ContextMenuSubTrigger>
        <ContextMenuSubContent>
  ...
  const content = (
    <ContextMenuItemContent
      ...
      shortcut={item.shortcutLabel || undefined}
    >
      {item.label}
    </ContextMenuItemContent>
  )
  ...
  if (item.checked !== undefined) {
    return (
      <ContextMenuCheckboxItem
        ...
        onCheckedChange={() =>
          onSelectItem ? onSelectItem(() => onExecute(item.command)) : onExecute(item.command)
        }
      >
        {content}
      </ContextMenuCheckboxItem>
...
export function CommandContextMenu({
  ...
  const [resolvedExtraItems, setResolvedExtraItems] = useState<
    readonly CommandContextMenuExtraItem[] | null
  >(null)
  const extraItemsRequestIdRef = useRef(0)
  const runtime = useCommandRuntime()
  const model = useResolvedCommandMenu(location)
  const mode = resolveMenuPresentationMode(location, preferredMode ?? 'nexus')
  ...
  const resolveExtraItemShortcutLabels = useCallback(
    (items: readonly CommandContextMenuExtraItem[]): readonly CommandContextMenuExtraItem[] => {
      ...
      const resolve = (
        source: readonly CommandContextMenuExtraItem[]
        ...
        source.map((item) => {
          ...
          return {
            ...item,
            shortcutLabel:
              item.shortcutLabel || resolveShortcutLabel(item.shortcutCommand) || undefined
          }
        })
  ...
  const displayedExtraItems = useMemo(
    () =>
      resolveExtraItemShortcutLabels(
        getExtraItems ? (resolvedExtraItems ?? pendingItems) : extraItems
      ),
  ...
  const handleNexusContextMenu = useCallback(
    (event: React.MouseEvent) => {
  ...
  const handleNexusOpenChange = useCallback(
    (open: boolean) => {
      onOpenChange?.(open)
  ...
  const handleNexusSelectItem = useCloseBeforeAction(handleNexusOpenChange)
      ...
      void Promise.resolve(nativeExtraItems)
        ...
        .then((resolvedNativeExtraItems) => {
          ...
          return window.api.command
            .showNativePopupMenu(nativeModel as never, anchor)
            .then((result) => {
              if (extraItemsRequestIdRef.current !== requestId) {
                return
              }
  ...
  return (
    <ContextMenu onOpenChange={handleNexusOpenChange}>
      <ContextMenuTrigger asChild onContextMenu={handleNexusContextMenu}>
        {children}
      </ContextMenuTrigger>
          ...
          {combinedItems.map((item, index) =>
            isExtraMenuItem(item) ? (
              <CommandContextMenuExtraItemView
                key={`extra-${item.id}`}
                item={item}
                onSelectItem={handleNexusSelectItem}
              />
            ) : (
              <CommandMenuItemView
                key={`${item.type}-${index}`}
 ...
 * model through Radix DropdownMenu in nexus mode, and (when location resolves
...
export function CommandPopupMenu({
  ...
  const runtime = useCommandRuntime()
  const model = useResolvedCommandMenu(location)
  const mode = resolveMenuPresentationMode(location, presentationMode ?? preferredMode ?? 'nexus')
  ...
  return (
    <DropdownMenu open={currentOpen} onOpenChange={handleNexusOpenChange}>
