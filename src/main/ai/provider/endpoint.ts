...
/**
 * Priority: `model.endpointTypes[0]` → `provider.defaultChatEndpoint` →
 * `undefined`. `getBaseUrl` applies its own fallback among `endpointConfigs`.
 */
export function resolveEffectiveEndpoint(provider: Provider, model: Model): ResolvedEndpoint {
  const endpointType = model.endpointTypes?.[0] ?? provider.defaultChatEndpoint
