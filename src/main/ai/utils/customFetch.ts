 ...
 */
const PROVIDER_USER_AGENT_HEADER = 'x-nexus-user-agent'
...
/**
 * Case-insensitively read a `User-Agent` out of a provider's `extraHeaders`.
 *
 * Exported so `providerToAiSdkConfig` can decide whether to install
 * {@link withForcedUserAgent} without duplicating the casing rules.
 */
export function pickUserAgent(headers: Record<string, string>): string | undefined {
  return resolveUserAgent(headers) ?? undefined
}

/**
 * Wrap a fetch so `userAgent` gets the FINAL word on the outgoing `User-Agent`.
 *
 * A `User-Agent` in `providerSettings.headers` cannot win on its own. The `ai`
 * package stamps `user-agent: ai/<version>` onto the CALL-level headers
 * (`withUserAgentSuffix(headers, 'ai/<v>')`), and the Anthropic model merges
 * call-level OVER provider-level (`combineHeaders(config.headers, headers,
 * betas)` — last writer wins). So a provider-level UA is silently dropped
 * before the request is even built; the observed wire value is the SDK's own
 * `ai/… ai-sdk/provider-utils/… runtime/…` chain with no trace of ours.
 *
 * The fetch boundary is the last hop the SDK cannot reach, so forcing it here
 * is the only placement that survives. Wrap OUTSIDE {@link customFetch} so the
 * value still rides the sentinel-header path past Chromium's UA overwrite.
 */
export function withForcedUserAgent(userAgent: string, inner: FetchFunction): FetchFunction {
  return (input, init) => {
