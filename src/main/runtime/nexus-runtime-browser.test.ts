...
import type { RuntimeBrowserCommandHost } from './nexus-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:nexus-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
...
describe('RuntimeBrowserCommands browser screencast', () => {
  ...
  it('waits for explicit worktree browser registration after requesting a hidden mount', async () => {
    const { RuntimeBrowserCommands } = await import('./nexus-runtime-browser')
    const send = vi.fn()
    const bridge = {
  ...
  it('sends the resolved isolated profile partition when creating a renderer tab', async () => {
    ...
    const send = vi.fn((channel: string, data: { requestId: string }) => {
      ...
      const handler = ipcMainOnMock.mock.calls.find(
        ...
        ((event: unknown, reply: { requestId: string; browserPageId?: string }) => void) | undefined
      handler?.({ sender: webContents } as never, {
        requestId: data.requestId,
    ...
    expect(send).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        sessionPartition: 'persist:nexus-browser-session-profile-isolated'
      })
    )
  })
