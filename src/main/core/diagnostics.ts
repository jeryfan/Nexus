/**
 * Opt-in performance diagnostics, gated by the `NEXUS_DIAGNOSTICS` env var.
 *
 ...
 * `NEXUS_DIAGNOSTICS=1 pnpm dev`.
 */

import { writeFileSync } from 'node:fs'
import { Session } from 'node:inspector'

export const DIAGNOSTICS_ENABLED = !!process.env.NEXUS_DIAGNOSTICS
...
export class CpuProfiler {
  ...
  private post(method: string, params?: Record<string, unknown>): Promise<any> {
    return new Promise((resolve, reject) => {
      this.session!.post(method, params as never, (err, result) =>
...
export function formatPhaseProfile(
  phase: string,
  spans: ServiceSpan[],
  lag: LagSummary,
  thresholdMs: number
  ...
  for (const sp of [...lag.spikes].sort((a, b) => a.at - b.at)) {
    lines.push(
      `  blocked=${sp.lag.toFixed(1).padStart(8)}ms  starting at offset=${sp.at.toFixed(1).padStart(8)}ms`
    )
  }
  return lines.join('\n')
}
