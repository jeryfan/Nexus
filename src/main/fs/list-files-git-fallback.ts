...
} from '@shared/fs/quick-open-readdir-walk'
import { fileListingCancellationError } from '@shared/fs/file-listing-cancellation'
 ...
 */
async function isInsideGitWorkTree(rootPath: string, signal?: AbortSignal): Promise<boolean> {
  if (signal?.aborted) {
...
export async function listFilesWithGit(
  ...
  const isGitWorkTree = await isInsideGitWorkTree(rootPath, signal)
  if (signal?.aborted) {
    throw fileListingCancellationError(signal)
  }
  if (!isGitWorkTree) {
  ...
  try {
    const runIgnoredPass = () =>
      ...
      runGitLsFiles(ignoredPass).catch(() => {
