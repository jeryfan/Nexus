 ...
 *
 * Pure JSON loading, caching, and lookups live in @nexus/provider-registry
...
import type { EndpointType, Modality, ModelCapability } from '@nexus/provider-registry'
import {
...
export type ReasoningProviderContext = Pick<
  Provider,
  'id' | 'presetProviderId' | 'defaultChatEndpoint'
>
...
export function resolveReasoningProfileFromRegistry(input: {
  ...
  const endpointDefault = input.endpointType
    ? DEFAULT_FORMAT_BY_ENDPOINT[input.endpointType]
    : undefined
...
export function applyCapabilityOverride(
  ...
  override:
    | { add?: ModelCapability[]; remove?: ModelCapability[]; force?: ModelCapability[] }
    | null
    | undefined
): ModelCapability[] {
  if (!override) {
...
function applyPresetAndOverride(
  presetModel: ProtoModelConfig,
  catalogOverride: ProtoProviderModelOverride | null
) {
...
class ProviderRegistryService {
  ...
  resolveAdapterFamilies(
    ...
    const presetProvider = presetProviderId
      ? this.findRegistryProvider(presetProviderId)
      : undefined
  ...
  getProviderDisplayMetadata(
    ...
    try {
      ...
      logger.warn('Failed to load provider display metadata', {
        providerId,
        presetProviderId,
        error
      })
  ...
  getProviderPreset(
    ...
    for (const field of new Set(fields)) {
      if (field === 'endpointConfigs') {
        ...
        result.models = presetProvider
          ? this.listProviderPresetModels(providerId, presetProvider)
          : []
      }
    }
  ...
  private getEffectiveProviderContext(providerId: string): ReasoningProviderContext {
    ...
    try {
      ...
      return {
        ...
        defaultChatEndpoint:
          provider.defaultChatEndpoint ?? registryProvider?.defaultChatEndpoint ?? undefined
      }
    } catch (error) {
      if (isDataApiError(error) && error.code === ErrorCode.NOT_FOUND) {
        return {
  ...
  resolveReasoningProfile(
    ...
    for (const providerId of providerIds) {
      for (const modelId of modelIds) {
        ...
        contract = effectiveEndpoint
          ? candidate?.reasoningContracts?.[effectiveEndpoint]
          : undefined
    ...
    const presetReasoning = this.getLoader().findModel(
  ...
  private listProviderPresetModels(
    ...
    for (const override of overrides) {
      ...
      const presetModel =
        loader.findModel(override.modelId) ?? synthesizePresetFromOverride(override)
      const reasoningProfile = this.resolveProfileForModelData(
        providerContext,
        presetModel,
        override,
  ...
  listProviderRegistryModels(options: ListProviderRegistryModelsOptions = {}): Model[] {
    ...
    if (options.providerId) {
      ...
      return presetProvider
        ? this.listProviderPresetModels(options.providerId, presetProvider, includeDisabled)
        : []
    }
    ...
    for (const override of overrides) {
      ...
      // models (modelscope's Tongyi-MAI/*, …) live
