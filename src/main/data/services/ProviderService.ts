...
import { deleteProviderLogo, getProviderLogoSrc } from '@main/services/providerLogoStore'
 ...
 */
export type ProviderLogoBindInput = { kind: 'key'; key: string } | { kind: 'default' }
export type UpdateProviderInput = UpdateProviderDto & { logo?: ProviderLogoBindInput }
...
function normalizeEndpointConfigs(
  ...
  return Object.fromEntries(
    Object.entries(endpointConfigs).map(([endpointType, config]) => [
      endpointType,
      EndpointConfigSchema.parse(config)
    ])
  )
}
...
function rowToRuntimeProvider(row: UserProviderRow): Provider {
  ...
  return {
    ...
    name: row.name,
    // Preset icon key stays on `logo`; uploaded bytes remain a main-process
    // storage detail and are exposed as a renderer-ready data URL.
    logo: row.logoKey ?? undefined,
    logoSrc: row.logoKey ? undefined : getProviderLogoSrc(row.providerId),
...
class ProviderService {
  ...
  list(query: ListProvidersQuery): Provider[] {
    ...
    if (query.endpointType !== undefined) {
      ...
      conditions.push(
        sql`json_extract(${userProviderTable.endpointConfigs}, ${'$.' + query.endpointType}) IS NOT NULL`
      )
    }
  ...
  create(dto: CreateProviderDto): Provider {
    ...
    const row = withSqliteErrors(
      () =>
        application.get('DbService').withWriteTx((tx) => {
          const values: NewUserProviderInput = {
            ...
            name: dto.name,
            logoKey: dto.logo?.kind === 'key' ? dto.logo.key : null,
            endpointConfigs,
      ...
      {
        unique: () =>
          DataApiErrorFactory.conflict(`Provider '${dto.providerId}' already exists`, 'Provider')
  ...
  update(providerId: string, dto: UpdateProviderInput): Provider {
    ...
    const row = application.get('DbService').withWriteTx((tx) => {
      ...
      if (dto.logo) {
        updates.logoKey = dto.logo.kind === 'key' ? dto.logo.key : null
      }
      ...
      if (dto.defaultChatEndpoint !== undefined)
        updates.defaultChatEndpoint = dto.defaultChatEndpoint
  ...
  getRotatedApiKey(providerId: string): string {
    ...
    const cache = application.get<CacheService>('CacheService')
  ...
  updateApiKey(
    ...
    const provider = db.transaction((tx) => {
      ...
      if (
        nextKeyValue &&
        existingKeys.some((entry, index) => index !== keyIndex && entry.key === nextKeyValue)
      ) {
        throw DataApiErrorFactory.conflict('API key already exists', 'API key')
      }
