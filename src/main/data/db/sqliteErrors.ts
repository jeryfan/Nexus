 ...
 * "forgot to rethrow" bug class at the type layer, which matters in Nexus
 * Studio's direct-db-access Service pattern where no IoC/AOP framework can
 * catch misses. If future Nexus architecture adopts such a framework,
...
export function defaultHandlersFor(resource: string, identifier: string): SqliteErrorHandlers {
  return {
    unique: () =>
      DataApiErrorFactory.conflict(`${resource} '${identifier}' already exists`, resource),
