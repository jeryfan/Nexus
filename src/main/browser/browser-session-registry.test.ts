...
import { NEXUS_BROWSER_PARTITION } from '../../shared/browser/constants'
import {
  DEFAULT_LOCAL_APP_PROFILE_ID,
  getBrowserDefaultPartition,
  getBrowserSessionPartition
} from '../../shared/browser/browser-profiles'
...
describe('BrowserSessionRegistry', () => {
  ...
  it('has a default profile on construction', () => {
    ...
    expect(defaultProfile.partition).toBe(NEXUS_BROWSER_PARTITION)
  })

  it('allows the default partition', () => {
    expect(browserSessionRegistry.isAllowedPartition(NEXUS_BROWSER_PARTITION)).toBe(true)
  })
  ...
  it('creates an isolated profile with a unique partition', () => {
    ...
    expect(profile!.partition).toMatch(/^persist:nexus-browser-session-/)
    expect(profile!.partition).not.toBe(NEXUS_BROWSER_PARTITION)
  ...
  it('resolves default partition for null/undefined profileId', () => {
    expect(browserSessionRegistry.resolvePartition(null)).toBe(NEXUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolvePartition(undefined)).toBe(NEXUS_BROWSER_PARTITION)
  })

  it('resolves default partition for unknown profileId', () => {
    expect(browserSessionRegistry.resolvePartition('nonexistent')).toBe(NEXUS_BROWSER_PARTITION)
  })
  ...
  it('strictly resolves known profile partitions without downgrading unknown profiles', () => {
    ...
    expect(browserSessionRegistry.resolveKnownPartition(null)).toBe(NEXUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition(undefined)).toBe(NEXUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition('default')).toBe(NEXUS_BROWSER_PARTITION)
  ...
  it('hydrates profiles from persisted data', () => {
    const fakeProfile = {
      ...
      partition: 'persist:nexus-browser-session-00000000-0000-0000-0000-000000000001',
      label: 'Hydrated',
  ...
  it('routes media permission requests through macOS TCC for isolated partitions', async () => {
    ...
    // after the user grants Camera/Microphone to the app.
  ...
  it('uses profile-owned partitions for non-default Nexus profiles', () => {
    const appProfileId = 'local-work'
    ...
    expect(browserSessionRegistry.getDefaultProfile().partition).toBe(
      getBrowserDefaultPartition(appProfileId)
    )
    expect(browserSessionRegistry.isAllowedPartition(NEXUS_BROWSER_PARTITION)).toBe(false)

    const profile = browserSessionRegistry.createProfile('isolated', 'Work Browser')
    expect(profile).not.toBeNull()
    expect(profile!.partition).toBe(
      getBrowserSessionPartition(appProfileId, profile!.id)
    )

    browserSessionRegistry.configureForBrowserProfile({
      appProfileId: DEFAULT_LOCAL_APP_PROFILE_ID,
