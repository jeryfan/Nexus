...
import { assertClipboardTextWriteWithinLimitWithYield } from '../../shared/browser/clipboard-text'
...
export class AgentBrowserBridge {
  ...
  async onTabClosed(webContentsId: number): Promise<void> {
    ...
    if (browserPageId) {
      const sessionName = `nexus-tab-${browserPageId}`
  ...
  async tabSwitch(
    ...
    return this.enqueueCommand(worktreeId, async () => {
      ...
      const resolvedPageId = browserPageId
      if (resolvedPageId) {
      ...
      async (_sessionName, target) => {
        ...
        try {
          ...
          if (!this.getWebContents(target.webContentsId)) {
            throw this.createPageUnavailableError(`nexus-tab-${target.browserPageId}`)
          }
  ...
  async setViewport(
    ...
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (_sessionName, target) => {
      ...
      // Why: agent-browser's `set viewport` has no `mobile` flag, so apply the emulation directly via CDP to honor the --mobile flag.
  ...
  async exec(command: string, worktreeId?: string, browserPageId?: string): Promise<unknown> {
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (sessionName) => {
      // Why: strip target/session flags from passthrough so a caller can't override the selected page or CDP proxy.
  ...
  private async enqueueTargetedCommand<T>(
    ...
    const sessionName = `nexus-tab-${target.browserPageId}`
  ...
  private async execAgentBrowser(
    ...
    // Why: a restarted named daemon auto-launches Chrome unless every invocation reasserts the CDP owner.
