...
import { GRAB_BUDGET } from '../../shared/browser/browser-grab-types'

const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__nexusGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-nexus-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-nexus-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-nexus-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-nexus-previous-display') || '';
    el.removeAttribute('data-nexus-previous-display');
  });
})()`
