...
describe('importCookiesFromFile', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'nexus-cookie-test-'))
...
describe('importCookiesFromBrowser Safari', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'nexus-safari-cookie-test-'))
    cookiesSetMock = vi.fn().mockResolvedValue(undefined)
    sessionFromPartitionMock.mockReset()
    sessionFromPartitionMock.mockReturnValue({
      cookies: { set: cookiesSetMock }
...
describe('importCookiesFromBrowser Chromium', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'nexus-chromium-cookie-test-'))
    cookiesSetMock = vi.fn().mockResolvedValue(undefined)
