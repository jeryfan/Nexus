...
import type { BrowserSnapshotRef } from '../../shared/browser/runtime-types'
  ...
  try {
    ...
    const { result } = (await sendCommand('Runtime.evaluate', {
      expression: `(() => {
        ...
        window.__nexusCursorInteractive = matchedElements;
    ...
    for (let i = 0; i < elements.length; i++) {
      try {
        const { result: objResult } = (await sendCommand('Runtime.evaluate', {
          expression: `window.__nexusCursorInteractive[${i}]`
    ...
    await sendCommand('Runtime.evaluate', {
      expression: 'delete window.__nexusCursorInteractive',
      returnByValue: true
    })
  } catch {
