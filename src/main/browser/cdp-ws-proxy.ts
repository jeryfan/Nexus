...
export class CdpWsProxy {
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // an app-specific product token (e.g. "Nexus/CdpWsProxy") would leak that
      // this is an embedded automation surface, which could affect downstream
      // detection heuristics — so respond with a plain Chrome product string.
  ...
  private handleClientMessage(client: WebSocket, raw: string): void {
    ...
    if (msg.method === 'Browser.getVersion') {
      // Why: an app-specific product token (e.g. "Nexus/Electron") would identify
      // this as an embedded automation surface to agent-browser. Use a generic
      // Chrome product string instead.
      const chromeVersion = process.versions.chrome ?? '134.0.0.0'
      this.sendResult(
        clientId,
        {
    ...
    // Use the same direct webContents reload path as the browser.reload command.
  ...
  private nextSyntheticPageSessionId(): string {
    ...
    return this.nextClientSessionOrdinal === 1
      ? 'nexus-proxy-session'
      : `nexus-proxy-session-${this.nextClientSessionOrdinal}`
  }
  ...
  private nextSyntheticBrowserSessionId(): string {
    ...
    return this.nextClientBrowserSessionOrdinal === 1
      ? 'nexus-proxy-browser-session'
      : `nexus-proxy-browser-session-${this.nextClientBrowserSessionOrdinal}`
  }
  ...
  private async reloadWithLifecycle(
    ...
    if (unsupportedParam) {
      this.sendError(
        clientId,
        `Page.reload parameter "${unsupportedParam}" is not supported for Nexus tab reloads`,
        client
      )
      return
    }
