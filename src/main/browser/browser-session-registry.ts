...
import {
  ...
  getBrowserPartitionSegment,
  getBrowserSessionPartition
} from '../../shared/browser/browser-profiles'
import type { BrowserSessionProfile, BrowserSessionProfileScope } from '../../shared/browser/types'
...
const LEGACY_BROWSER_SESSION_PARTITION_RE =
  /^persist:nexus-browser-session-[\da-f-]{8}-[\da-f-]{4}-[\da-f-]{4}-[\da-f-]{4}-[\da-f-]{12}$/
...
class BrowserSessionRegistry {
  ...
  private activeAppProfileId = DEFAULT_LOCAL_APP_PROFILE_ID
  private metadataPathOverride: string | null = null
  private defaultPartition = NEXUS_BROWSER_PARTITION

  constructor() {
    this.resetDefaultProfile()
  }

  configureForBrowserProfile(options: BrowserSessionRegistryProfileOptions): void {
    this.activeAppProfileId = options.appProfileId
    this.metadataPathOverride = join(options.profileDirectory, BROWSER_SESSION_META_FILE_NAME)
    this.defaultPartition = getBrowserDefaultPartition(options.appProfileId)
    this.profiles.clear()
    this.resetDefaultProfile()
  }
  ...
  resolveKnownPartition(profileId: string | null | undefined): string | null {
    if (!profileId) {
      // Why: use the active Nexus profile's default partition, not the legacy constant, or profiles resolve local-default's cookie jar.
      return this.defaultPartition
    }
  ...
  createProfile(scope: BrowserSessionProfileScope, label: string): BrowserSessionProfile | null {
    ...
    const partition = getBrowserSessionPartition(this.activeAppProfileId, id)
  ...
  private isProfileOwnedSessionPartition(partition: string): boolean {
    if (
      this.activeAppProfileId === DEFAULT_LOCAL_APP_PROFILE_ID &&
      LEGACY_BROWSER_SESSION_PARTITION_RE.test(partition)
    ) {
      return true
    }

    const segment = getBrowserPartitionSegment(this.activeAppProfileId)
    const prefix = `persist:nexus-profile-${segment}-browser-session-`
