...
function installModuleMocks(
  ...
  const sessionFromPartitionMock = vi.fn((partition: string) => ({
    partition,
    setUserAgent: vi.fn(),
    getUserAgent: vi.fn(() => 'Mozilla/5.0 Electron/31 Nexus'),
...
describe('BrowserSessionRegistry persistence', () => {
  ...
  it('migrates and consumes legacy pendingCookieDbPath into default partition replay', async () => {
    ...
    expect(fsState.present.has('/user-data/Partitions/nexus-browser/Cookies')).toBe(true)
  })

  it('replays pending cookies into an existing Network database', async () => {
    const stagedPath = '/staged/network-import'
    const networkPath = '/user-data/Partitions/nexus-browser/Network/Cookies'
    const legacyPath = '/user-data/Partitions/nexus-browser/Cookies'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
  ...
  it('persists new browser session profiles under the active Nexus profile directory', async () => {
    const fsState = createFsState()
    ...
    browserSessionRegistry.configureForBrowserProfile({
      appProfileId: 'local-work',
  ...
  it('merges partition-keyed pending entries without clobbering unrelated entries', async () => {
    ...
    browserSessionRegistry.setPendingCookieImport('persist:nexus-browser', '/staged/default')
    browserSessionRegistry.setPendingCookieImport(
      'persist:nexus-browser-session-aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      '/staged/imported'
    )
    ...
    expect(written.pendingCookieImports).toEqual({
      'persist:nexus-browser': '/staged/default',
      'persist:nexus-browser-session-aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa': '/staged/imported'
    })
  })

  it('clears only the requested partition and unlinks its staged database files', async () => {
    const otherPartition = 'persist:nexus-browser-session-bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
    ...
    const written = JSON.parse(fsState.files.get(META_PATH) ?? '{}')
    expect(written.pendingCookieImports).toEqual({ 'persist:nexus-browser': '/staged/default' })
  ...
  it('drops the legacy pointer when the default partition is the one cleared', async () => {
    const otherPartition = 'persist:nexus-browser-session-cccccccc-cccc-4ccc-8ccc-cccccccccccc'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
    ...
    browserSessionRegistry.clearPendingCookieImport('persist:nexus-browser')
  ...
  it('is a no-op when the partition has no pending import', async () => {
    ...
    seedMeta(fsState, {
      ...
      userAgentByPartition: {},
      pendingCookieDbPath: '/staged/default',
      pendingCookieImports: { 'persist:nexus-browser': '/staged/default' },
      profiles: []
    })
    ...
    browserSessionRegistry.clearPendingCookieImport('persist:nexus-browser-session-unknown')
  ...
  it('restores persisted UA for non-default partitions', async () => {
    const importedPartition = 'persist:nexus-browser-session-11111111-1111-4111-8111-111111111111'
    ...
    seedMeta(fsState, {
      ...
      userAgent: defaultUa,
      userAgentByPartition: {
        'persist:nexus-browser': defaultUa,
  ...
  it('sets up default-partition policies on restore', async () => {
    ...
    const defaultSessions = sessionFromPartitionMock.mock.results
      .filter((_, idx) => sessionFromPartitionMock.mock.calls[idx]?.[0] === 'persist:nexus-browser')
      .map((r) => r.value)
  ...
  it('notifies when default-partition media permission is denied', async () => {
    ...
    const defaultSession = sessionFromPartitionMock.mock.results.find(
      (_, idx) => sessionFromPartitionMock.mock.calls[idx]?.[0] === 'persist:nexus-browser'
    )?.value
    const requestHandler = defaultSession.setPermissionRequestHandler.mock.calls[0][0]
  ...
  it('keeps failed partition replay pending and removes unrelated missing entries', async () => {
    const importedPartition = 'persist:nexus-browser-session-22222222-2222-4222-8222-222222222222'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
      ...
      pendingCookieImports: {
        ...
        'persist:nexus-browser': '/staged/missing'
      },
      profiles: [
        {
