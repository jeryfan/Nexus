...
} from '../../shared/browser/window-shortcut-policy'
import { readGuestNavigationState } from './browser-guest-navigation-state'
import { keybindingMatchesAction, type KeybindingOverrides } from '../../shared/browser/keybindings'
import { FLOATING_TERMINAL_WORKTREE_ID } from '../../shared/browser/constants'
import type { BrowserPageZoomDirection } from '../../shared/browser/browser-page-zoom'
import {
  ModifierDoubleTapDetector,
  toModifierDoubleTapEvent
} from '../../shared/browser/modifier-double-tap-detector'
import type { BrowserFindSource } from '../../shared/browser/browser-find-source'
...
export function setupGrabShortcutForwarding(args: {
  ...
  const handler = (event: Electron.Event, input: Electron.Input): void => {
    ...
    if (
      ...
      // Why: a focused guest swallows bare keys; during an active grab pick, plain C/S are the app's copy/screenshot, not page typing.
...
export function setupGuestShortcutForwarding(args: {
  ...
  const forwardShortcutInput = (
    ...
    if (action?.type === 'zoom') {
      // Why: focused guest key events never reach the renderer-owned webview ref that applies the app's page zoom.
