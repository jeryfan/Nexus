...
function isGuestCancellationPayload(rawPayload: unknown): boolean {
  ...
  if (payload.__nexusCancelled === true) {
    return true
  }
...
export class BrowserGrabSessionController {
  ...
  awaitGrabSelection(
    ...
    return new Promise<BrowserGrabResult>((resolve) => {
      ...
      // __nexusGrabResolve() which is wired by the 'awaitClick' script to
      ...
      const awaitGuestClick = async (): Promise<void> => {
        try {
          ...
          // Why: the guest wraps right-click results in { __nexusContextMenu, payload }
          // so the renderer can show the full action dropdown instead of auto-copying.
          const isContextMenu =
            '__nexusContextMenu' in (rawPayload as Record<string, unknown>) &&
            (rawPayload as Record<string, unknown>).__nexusContextMenu === true
