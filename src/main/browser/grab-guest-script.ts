...
// arm: install the overlay + hover tracking; state lives on window.__nexusGrab so finalize/teardown can reach it.
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__nexusGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__nexusGrab) {
    try {
      if (typeof window.__nexusGrab.cleanup === 'function') {
        window.__nexusGrab.cleanup();
      }
    } catch(e) {}
    delete window.__nexusGrab;
  }
...
const AWAIT_CLICK_SCRIPT = `(async function() {
  ...
  return await new Promise(function(resolve, reject) {
    ...
    function onContext(e) {
      ...
      if (!payload) return;
      grab.freezeHighlight();
      resolve({ __nexusContextMenu: true, payload: payload });
    }
...
const TEARDOWN_SCRIPT = `(function() {
  ...
  // it with the __nexusCancelled marker so the executeJavaScript call in main
