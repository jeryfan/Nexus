...
import { NEXUS_BROWSER_BLANK_URL } from '../../shared/browser/constants'
import {
...
} from '../../shared/browser/browser-annotation-viewport-bridge'
...
function releaseAutomationVisibilityToken(renderer: Electron.WebContents, token: string): void {
  ...
  renderer
    .executeJavaScript(
      `(function() {
        var bridge = window.__nexusBrowserAutomationVisibility;
...
export class BrowserManager {
        ...
        .executeJavaScript(
          `(function() {
            ...
            if (
              ...
              // Why: Nexus remembers the last browser workspace/page even when
  ...
  attachGuestPolicies(
    ...
    // Why: only the primary embedded browser converts new-tab clicks to Nexus tabs; OAuth child windows keep native link behavior.
    const clickedLinkFrameName = inheritedOwnerContext
      ? null
      : `__nexus_clicked_link_foreground_${randomUUID()}`
    ...
    const installIframeClickedLinkRouting = (frame: Electron.WebFrameMain): void => {
      ...
      const name = `__nexus_clicked_link_iframe_foreground_${randomUUID()}`
    ...
    guest.setWindowOpenHandler(({ url, frameName }) => {
      ...
      if (isClickedLink) {
        if (browserTabId && browserUrl && this.openLinkInNexusTab(browserTabId, browserUrl)) {
          this.forwardOrQueuePopupEvent(guest.id, {
            origin: safeOrigin(browserUrl),
            action: 'opened-in-nexus'
          })
        }
      ...
      const canOpenAsChild = Boolean(externalUrl || browserUrl === NEXUS_BROWSER_BLANK_URL)
      if (browserTabId && canOpenAsChild) {
        ...
        return {
          ...
          // Why: default child windows lack an address bar; host in an origin-bar window so the destination is verifiable.
  ...
  // Why: headless CLI serve has no <webview> window; back pages with offscreen WebContents and skip the webview-only setup.
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Nexus is shutting down.')
    }
  ...
  // Why: guests are isolated from the app's preload bridge, so main owns the devtools escape hatch after a tab→guest lookup.
  ...
  private openLinkInNexusTab(browserTabId: string, rawUrl: string): boolean {
    const renderer = this.resolveRendererForBrowserTab(browserTabId)
    if (!renderer) {
      return false
    }
    const normalizedUrl = normalizeBrowserNavigationUrl(rawUrl)
    if (!normalizedUrl || normalizedUrl === NEXUS_BROWSER_BLANK_URL) {
      return false
    }
    // Why: only the renderer owns the app's worktree/tab model; main forwards a validated URL, never letting guest content mutate it.
    renderer.send('browser:open-link-in-nexus-tab', {
      browserPageId: browserTabId,
      url: normalizedUrl
    })
    return true
