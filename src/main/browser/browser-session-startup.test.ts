...
function installRegistryMock(): {
  configureForBrowserProfileMock: ReturnType<typeof vi.fn>
  ...
  const configureForBrowserProfileMock = vi.fn()
  ...
  vi.doMock('./browser-session-registry', () => ({
    browserSessionRegistry: {
      configureForBrowserProfile: configureForBrowserProfileMock,
  ...
  return {
    configureForBrowserProfileMock,
...
describe('initializeBrowserSessionsForApp', () => {
  ...
  it('configures the active Nexus profile before replaying browser sessions', async () => {
    const {
      configureForBrowserProfileMock,
      applyPendingCookieImportMock,
      initializeBrowserSessionsFromPersistedStateMock
    ...
    expect(configureForBrowserProfileMock).toHaveBeenCalledWith({
      appProfileId: 'local-work',
      profileDirectory: '/profiles/local-work'
    })
    expect(configureForBrowserProfileMock.mock.invocationCallOrder[0]).toBeLessThan(
