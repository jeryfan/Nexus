 ...
 */
export async function setProviderLogo(providerId: string, image: LogoImageIntent): Promise<void> {
  // Validate the owner before touching the filesystem.
  providerService.getByProviderId(providerId)

  const previous = readProviderLogo(providerId)
  try {
    if (image.kind === 'image') {
      writeProviderLogo(providerId, image.data)
      providerService.update(providerId, { logo: { kind: 'default' } })
      return
    }

    deleteProviderLogo(providerId)
    providerService.update(providerId, {
      logo: image.kind === 'key' ? { kind: 'key', key: image.key } : { kind: 'default' }
    })
  } catch (error) {
    if (previous) {
      writeProviderLogo(providerId, previous)
    } else {
      deleteProviderLogo(providerId)
    }
    throw error
