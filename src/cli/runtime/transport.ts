// 裁剪: 删除 orchestration 信封字段
// （orchestrationCapability / orchestrationContractVersion / orchestrationRequestId /
// compatibilityInvocationId / orchestrationCompatibilityEvidence，orchestration 命令域，
// 未迁移），请求行只保留 id/authToken/method/params。本地 unix socket / named pipe
// 分帧、keepalive、envelope schema 校验逻辑原样保留。
import { createConnection } from 'node:net'
import { randomUUID } from 'node:crypto'
import { findTransport, type RuntimeMetadata } from '../../shared/browser/runtime-bootstrap'
...
import { MAX_TIMER_DELAY_MS, isSafeTimerDelayMs } from '../../shared/browser/timer-delay'
  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Nexus runtime metadata.'
        )
      )
      return
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new RuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the Nexus runtime to respond.'
        )
      )
    }, timeoutMs)
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Nexus app. Restart Nexus and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The Nexus runtime closed the connection before responding. Restart Nexus and try again.'
        )
      })
    })
    socket.on('data', (chunk: string) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Nexus runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Nexus ships on
        // Node 24 via Electron and the standalone CLI targets the same
        // major). See §7 risk #9.
        ...
        // the decode boundary so version skew between the CLI and the Nexus
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Nexus runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Nexus runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
