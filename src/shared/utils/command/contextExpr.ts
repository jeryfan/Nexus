...
class Parser {
  ...
  private parseEquality(): ContextExpr {
    ...
    return operator === '=='
      ? { type: 'equals', key: left.key, value }
      : { type: 'notEquals', key: left.key, value }
  }
...
export const evaluateContextExpr = (
  expr: ContextExpr | undefined,
  ...
  switch (expr.type) {
    case 'key':
      return singleConstraintTerm(
        expr.key,
        createConstraint({ polarity: negated ? 'falsy' : 'truthy' })
      )
    case 'equals':
      return singleConstraintTerm(
        expr.key,
        negated
          ? createConstraint({ notEquals: [expr.value] })
          : createConstraint({ equals: expr.value })
      )
    case 'notEquals':
      return singleConstraintTerm(
        expr.key,
    ...
    case 'or': {
      ...
      return expr.exprs.reduce<ConstraintTerm[] | undefined>(
        (acc, item) => concatDnfTerms(acc, toDnfTerms(item)),
        []
      )
    }
  }
 ...
 */
export const canContextExprsOverlap = (
  left: ContextExpr | undefined,
  right: ContextExpr | undefined
): boolean => {
