...
const registerMenuContribution = (
  ...
  if (!findCommandDefinition(contribution.command)) {
    throw new Error(
      `Cannot register menu contribution for unknown command: ${contribution.command}`
    )
  }
  return {
    ...contribution,
...
const resolveContributions = (
  ...
  const matched = contributions
    ...
    .sort(
      (a, b) =>
        a.group.localeCompare(b.group) || a.order - b.order || a.command.localeCompare(b.command)
    )
