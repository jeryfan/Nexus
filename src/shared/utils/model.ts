 ...
 *    `@nexus/provider-registry` (creator-declared data).
 */

import { MODALITY, VENDOR_PATTERNS } from '@nexus/provider-registry'
...
export const isEmbeddingModel = (model: Model): boolean =>
...
export const isSupportTopPModel = (model: Model): boolean =>
  model.parameterSupport?.topP?.supported !== false
...
// `@nexus/provider-registry`. Do NOT inline new regex here —
...
export const isQwenMTModel = (model: Model): boolean =>
  getLowerBaseModelName(getRawModelId(model)).includes('qwen-mt')
...
export const isGPT5SeriesReasoningModel = (model: Model): boolean =>
  isGPT5SeriesModel(model) && isReasoningModel(model)
