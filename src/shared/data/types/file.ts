/**
 * File-domain data types — the cross-process shapes for Nexus-managed files.
 *
 ...
 *
 * - `internal`: Nexus owns the content, stored at `{userData}/Data/Files/{id}.{ext}`.
 *   `name` / `ext` / `size` are authoritative truth (kept in sync by atomic writes).
 * - `external`: Nexus only references a user-provided path (`externalPath`).
 ...
 *
 * External files can change outside Nexus at any time (user edits, another app
...
/**
 * Internal entry — Nexus owns the content at `{userData}/Data/Files/{id}.{ext}`.
 *
...
export const InternalEntrySchema = z.strictObject({
  ...
  /**
   * File size in bytes. Internal files are written atomically by Nexus, so
...
/**
 * External entry — Nexus references a user-provided path.
 *
 ...
 * outside Nexus at any time so no DB size snapshot is kept (live values
...
/**
 * FileHandle — unified reference to any file accessible by Nexus.
 *
 ...
 */
export const createRefSchema = <T extends BusinessRefShape>(
  shape: T
): z.ZodObject<typeof refCommonFields & T> =>
  z.object({
    ...refCommonFields,
    ...shape
  })
