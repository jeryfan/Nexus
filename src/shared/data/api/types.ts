 ...
 */
export function isCustomStatusResult<T>(
  result: unknown
): result is { data: T; status: SuccessStatusCode } {
  return (
    result !== null &&
    typeof result === 'object' &&
    'status' in result &&
    typeof (result as Record<string, unknown>).status === 'number' &&
    Object.values(SuccessStatus).includes(
      (result as Record<string, unknown>).status as SuccessStatusCode
    )
  )
}
 ...
 * export type ApiSchemas = AssertValidSchemas<ProviderSchemas & ModelSchemas>
 *
...
export interface DataRequest<T = any> {
  ...
  method: HttpMethod
  /** API path (e.g., '/providers', '/providers/openai') */
  path: string
...
export type InferPaginationMode<R> =
  R extends OffsetPaginationResponse<any>
    ? 'offset'
...
> = TPath extends keyof ApiSchemas
...
export type ApiHandler<Path extends ApiPaths, Method extends ApiMethods<Path>> = (
  params: (HasRequiredParams<Path, Method> extends true
    ...
    (HasRequiredQuery<Path, Method> extends true
      ...
      : { query?: ApiQuery<Path, Method> }) &
    (HasRequiredBody<Path, Method> extends true
 ...
 *
 * Given a schema subset (e.g. `ProviderSchemas`), produces the handler record
 ...
 * path (e.g. '/providers/:providerId/models:reconcile') has no readable state to converge on.
 */
 ...
 *
 * `InferPaginationMode` alone is not enough — current collections such as
 * `/providers` return bare arrays. Wrapper-object list responses that do NOT
 * extend a pagination type deliberately degrade to
...
export type CollectionGetPaths = {
  [K in GetMethodApiPaths]: ResponseForPath<K, 'GET'> extends
    readonly unknown[] | CursorPaginationResponse<any> | OffsetPaginationResponse<any>
    ? K
    : never
}[GetMethodApiPaths]
 ...
 *   returns (for example provider ids from `/providers`). Plural — a
