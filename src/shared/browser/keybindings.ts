...
export type KeybindingScope =
  'global' | 'tabs' | 'terminal' | 'browser' | 'editor' | 'fileExplorer' | 'composer' | 'settings'

export type KeybindingContext = 'app' | 'terminal' | 'browser'

export type KeybindingPlatform = 'darwin' | 'linux' | 'win32'

export type TerminalShortcutPolicy = 'nexus-first' | 'terminal-first'
...
export const KEYBINDING_DEFINITIONS: readonly KeybindingDefinition[] = [
  ...
  },
  {
    ...
    // Why: macOS uses Shift+Space as an input-source shortcut; Nexus otherwise rejects Shift-only bindings to avoid stealing typed text.
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'nexus-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: Nexus-first keeps app shortcuts inside terminals; terminal-first is the escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'nexus-first') {
    return true
  }
