...
// 裁剪: getDefaultSettings no longer sets `sourceControlAi` — the
// source-control-ai module (settings normalization + generation logic, ~2.4k
// lines with commit-message-agent-spec/source-control-ai-actions deps) belongs
// to the unmigrated source-control AI domain. The Settings.sourceControlAi field
// was removed from ./types as well; persistence merge should treat it as absent.
// Also no longer set here (matching fields trimmed from ./types for unmigrated
// domains): Settings.terminalCustomThemes, .claudeAgentTeamsMode,
// .nativeChatSessionOptions, .githubProjects;
// PersistedState.sshTargets,
// .sshRemotePtyLeases, .sshPtyConsumerRecoveries, .migrationUnsupportedPtyEntries,
// .automations, .automationRuns, .featureInteractionTelemetryBuckets;
// PersistedUIState.workspaceCleanup, .featureTipsSeenIds, .featureInteractions,
...
function getDefaultWorkspaceDir(homeDir: string): string {
  ...
  return [trimmedHomeDir, 'nexus', 'workspaces'].join(separator)
}
...
export function getDefaultSettings(homedir: string): GlobalSettings {
  return {
    ...
    // Why: Nexus-first keeps core shortcuts working from a focused terminal; TUI-ownership users opt in.
    terminalShortcutPolicy: 'nexus-first',
