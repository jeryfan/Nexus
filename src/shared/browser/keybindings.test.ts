...
describe('keybindings', () => {
  it('accepts bounded plugin command action IDs and rejects malformed variants', () => {
    expect(isKeybindingActionId('plugin:nexus-samples.tasks/open')).toBe(true)
    expect(isKeybindingActionId('plugin:nexus-samples.tasks/task.open-latest')).toBe(true)
    expect(isKeybindingActionId('plugin:tasks/open')).toBe(false)
    expect(isKeybindingActionId('plugin:nexus-samples.tasks/../open')).toBe(false)
    expect(isKeybindingActionId(`plugin:nexus-samples.tasks/${'a'.repeat(401)}`)).toBe(false)
  })
  ...
  it('keeps Nexus-first terminal context backward compatible', () => {
    const ctrlP = {
      key: 'p',
      code: 'KeyP',
      control: true,
