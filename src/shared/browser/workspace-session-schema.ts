...
// 裁剪: sleepingAgentSessionsByPaneKeySchema import and the
// sleepingAgentSessionsByPaneKey session field removed — workspace-session-sleeping-agents
// depends on the unmigrated agent-session-resume / agent-hook-relay (relay) domain.
// Zod strips the unknown key on parse, so persisted sessions remain readable.
...
export type ParsedWorkspaceSession =
  { ok: true; value: WorkspaceSessionState } | { ok: false; error: string }
