...
// 裁剪: imports and fields referencing the unmigrated agent-status
// (agent-status-types / agent-session-resume / codex-startup-delivery), mobile
// (mobile-markdown-document), and remote/relay (remote-runtime-shared-control-types,
// remote-server-update) domains were removed. Affected exports: RuntimeStatus
// (remoteUpdateSupport, remoteControl), CliStatusResult (remoteUpdateSupport),
// RuntimeSyncWindowGraphResult (agentOrchestrationByPaneKey),
// RuntimeMobileSessionTerminalTab (agentStatus), RuntimeTerminalCreateBaseRequestPayload
// (launchConfig, resumeProviderSession, startupCommandDelivery),
// RuntimeWorktreeAgentRow (state, agentType), and the RuntimeMarkdown*TabResult re-export.
import type {
  BaseRefSearchResult,
...
export type RuntimeTerminalDriverState =
  { kind: 'idle' } | { kind: 'desktop' } | { kind: 'mobile'; clientId: string }
...
export type CliRuntimeState =
  'not_running' | 'starting' | 'ready' | 'graph_not_ready' | 'stale_bootstrap'
...
export type RuntimeWorktreePsConditionalResult =
  RuntimeWorktreePsSnapshotResult | RuntimeWorktreePsUnchangedResult
