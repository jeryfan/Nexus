...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  ...
  'claude-agent-teams': {
    // Why: an app-provided launch mode, not a separate binary; detection follows the Nexus CLI.
    detectCmd: 'nexus',
    detectCmdAliases: ['nexus-dev', 'nexus-ide'],
    // Why: require Claude too so fresh installs (Nexus shim always present) don't report Agent Teams without an agent CLI.
    detectRequiredCommands: ['claude'],
    // Why: Windows/WSL use Claude's in-process Agent Teams fallback, not this native-pane/tmux-shim wrapper.
    detectUnsupportedRuntimes: ['win32', 'wsl'],
    launchCmd: 'nexus claude-teams',
    launchCmdByPlatform: {
  ...
  pi: {
    ...
    expectedProcess: 'pi',
    promptInjectionMode: 'argv',
    // Why: pi has no `--prefill` and paste-after-ready races its long startup; a pi extension seeds this env var instead.
    draftPromptEnvVar: 'NEXUS_PI_PREFILL',
  ...
  omp: {
    ...
    expectedProcess: 'omp',
    promptInjectionMode: 'argv',
    draftPromptEnvVar: 'NEXUS_OMP_PREFILL'
  },
  gemini: {
    detectCmd: 'gemini',
    launchCmd: 'gemini',
  ...
  hermes: {
    detectCmd: 'hermes',
    // Why: bare `hermes` opens the classic REPL; `--tui` starts the full-screen agent UI Nexus hosts.
...
export function getTuiAgentLaunchCommand(
  ...
  // Why: the local-only `-ide` command rename must not leak to Linux remotes, whose relay shim keeps the plain CLI command name.
