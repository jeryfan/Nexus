...
// 裁剪: imports, re-exports, type aliases, and interface fields
// referencing unmigrated domains were removed (each is optional unless noted):
// - ssh-types (PersistedState.sshTargets [required], .sshRemotePtyLeases [required],
//   .removedSshTargetTombstones, .sshPtyConsumerRecoveries)
// - automations-types (AutomationWorkspaceProvenance,
//   AutomationWorkspaceProvenanceRequest types and all fields using them,
//   PersistedState.automations/.automationRuns [required])
// - release-channel (UpdateCheckOptions.channel, PersistedUIState.releaseChannelOverride,
//   ReleaseBuildListResult type)
// - github-project-types / gitlab-types / jira-types (Settings.githubProjects/.gitlabProjects,
//   gitlab-types and jira-types re-export blocks)
// - agent-status-types (NotificationDispatchRequest.agentType/.agentState,
//   PersistedState.migrationUnsupportedPtyEntries [required])
// - workspace-cleanup (PersistedUIState.workspaceCleanup)
// - large-diff-render-limit (GitDiffTextResult.largeDiffRenderLimit)
// - feature-tips / contextual-tours / feature-interactions
//   (PersistedUIState.featureTipsSeenIds/.contextualToursSeenIds/.featureInteractions,
//   PersistedState.featureInteractionTelemetryBuckets)
// - repo-icon (Project.repoIcon, Repo.repoIcon)
// - source-control-ai-types (ProjectHostSetup.sourceControlAi, Repo.sourceControlAi,
//   Settings.sourceControlAi)
// - codex-startup-delivery (WorktreeStartupLaunch.startupCommandDelivery)
// - telemetry-events (WorktreeStartupLaunch.telemetry)
// - agent-session-resume (WorkspaceSessionState.sleepingAgentSessionsByPaneKey,
//   WorktreeStartupLaunch.launchConfig)
// - claude-agent-teams-tmux-compat (Settings.claudeAgentTeamsMode)
// - terminal-custom-themes (Settings.terminalCustomThemes)
// - git-fork-sync (git-fork-sync re-export, Repo.forkSyncMode)
// - git-remote-identity (Project.gitRemoteIdentity, Repo.gitRemoteIdentity)
// - native-chat-session-options (Settings.nativeChatSessionOptions)
// - task-source-context (FolderWorkspace/Worktree/WorktreeMeta/CreateWorktreeArgs.linkedTaskSourceContext)
// - setup-runner-command (WorktreeSetupLaunch.shell; added after our copy base,
//   the module remains unmigrated)
// - release-channel (UpdateSource was re-derived from DedicatedRepoChannel; we keep the
//   pre-#12051 'local' | 'hourly' shape because release-channel is unmigrated)
...
export type ShellHydrationFailureReason =
  'none' | 'no_shell' | 'timeout' | 'spawn_error' | 'empty_path'
...
export type ProjectHostSetupMethod =
  'legacy-repo' | 'imported-existing-folder' | 'cloned' | 'provisioned'
...
export type Repo = {
  ...
  /** Controls whether worktrees Nexus did not create appear in the sidebar. */
...
export type WorkspaceScope =
  { type: 'worktree'; worktreeId: string } | { type: 'folder'; folderWorkspaceId: string }
...
export type Worktree = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** Set once when Nexus creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Nexus originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Nexus should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `nexus worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** Nexus terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Nexus added this remote while preparing a fork-PR worktree. */
...
export type WorktreeMeta = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** See {@link Worktree.createdAt}. Persisted to nexus-browser-session.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to nexus-browser-session.json. */
  createdWithAgent?: TuiAgent
  ...
  /** Intended create base for stale-base probes. Persisted metadata, not UI drift state. */
  baseRef?: string
  /** True when Nexus checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Nexus creates the worktree. */
  nexusCreatedAt?: number
  nexusCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Nexus created the worktree. */
  nexusCreationWorkspaceLayout?: BrowserWorkspaceLayout
  ...
  priorWorktreeIds?: string[]
  mobileDiffReview?: MobileDiffReviewState
  /** System-owned provenance for workspaces created via `nexus worktree create`. */
  cliProvenance?: CliWorkspaceProvenance
}

export type WorktreeOwnership = 'nexus-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
...
// existing persistence layer writes them to nexus-browser-session.json automatically.
...
export type TerminalTab = {
   ...
   *  Why: agent CLIs overwrite the live title via OSC updates, but Nexus still
  ...
  startupCwd?: string
  /** Why: the coding-harness agent Nexus launched in this tab. Lets the tab bar
...
export type BrowserCertificateProceedFailureReason =
  'expired' | 'changed' | 'ineligible' | 'missing' | 'navigated'

export type BrowserCertificateProceedResult =
  { ok: true } | { ok: false; reason: BrowserCertificateProceedFailureReason }
...
export type BrowserWorkspace = {
  id: string
  worktreeId: string
  /** Stable display label for the outer Nexus tab ("Browser 1", "Browser 2", …).
  ...
  // Nexus tab strip still renders one workspace entry. Mirror the active page's
...
export type BrowserSessionProfileSource = {
  browserFamily:
    'chrome' | 'chromium' | 'arc' | 'edge' | 'firefox' | 'safari' | 'comet' | 'helium' | 'manual'
  profileName?: string
  importedAt: number
}
...
export type GitHubPRRefreshSkippedReason =
  'fresh' | 'not-git' | 'bare' | 'archived' | 'disconnected' | 'remote' | 'rate-limit' | 'capacity'
...
export type GitHubWorkItemDetails = {
  // Why: main-process doesn't know Nexus's Repo.id, so this inner item omits
...
export type GitHubCreateIssueResult =
  { ok: true; number: number; url: string; bodySaveWarning?: string } | { ok: false; error: string }
...
export type GetRateLimitResult =
  { ok: true; snapshot: GitHubRateLimitSnapshot } | { ok: false; error: string }
...
export type BrowserHooks = {
  ...
  defaultTabs?: BrowserDefaultTabTemplate[] // Terminal tabs to create once for a new worktree
  environmentRecipes?: BrowserVmRecipe[] // Project-scoped per-workspace environment recipes
  environmentRecipeDiagnostics?: BrowserVmRecipeDiagnostic[] // Non-fatal validation issues from environmentRecipes
  worktree?: BrowserWorktreeDefaults // Project-scoped defaults applied when a worktree is created
}

export type BrowserWorktreeDefaults = {
...
export type BrowserDefaultTabTemplate = {
  title?: string
  color?: string
  command?: string
}

export type BrowserVmRecipe = {
  id: string
  name: string
  create: string
  description?: string
...
export type BrowserVmRecipeDiagnostic = {
  index: number
  field?: string
  message: string
}
...
export type WorktreeDefaultTabsLaunch = {
  tabs: BrowserDefaultTabTemplate[]
  runCommands: boolean
}
...
export type LinuxPackageInstallFailureReason =
  'authentication-agent-unavailable' | 'authentication-denied' | 'package-install-failed'
 ...
 *  system-default (activeAccountId:null) Codex account. Nexus reads this to
...
/** All AI coding agents Nexus knows how to launch. Used for the agent picker in the new-workspace
 *  flow and for the default-agent setting. Extend this union as new agents are added. */
export type TuiAgent =
  | 'claude' // Claude Code
  | 'claude-agent-teams' // Claude Code Agent Teams via Nexus native panes
...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: BrowserWorkspaceLayout[]
  ...
  /** When enabled, Nexus renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (Nexus survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the Nexus app name in the titlebar. */
  ...
  /** Only toggles the sidebar shortcut; Nexus Mobile stays reachable from Settings. */
  ...
  /** Nexus-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.nexus/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so Nexus can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks Nexus uses. */
  ...
  /** When true, Nexus requests local awake assertions while hook-reported agents are working. */
...
export type BrowserWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
...
export type WorktreeCardProperty =
  ...
  | 'pr'
  | 'automation'
  // Badge marking workspaces created through `nexus worktree create`.
  | 'cli'
  | 'comment'
  | 'ports'
...
export type TopLevelView =
  'terminal' | 'settings' | 'tasks' | 'activity' | 'automations' | 'space' | 'skills' | 'mobile'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `nexus worktree create`. */
  ...
  /** Whether Nexus already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "Nexus is still running" notice shows only once. */
  ...
  /** Once the user has starred Nexus (any entry point), permanently suppress the nag. */
  ...
  trustedBrowserHooks?: PersistedTrustedBrowserHooks
...
export type PersistedTrustedBrowserHookEntry = {
  contentHash: string
  approvedAt: number
}

export type PersistedTrustedBrowserHookRepo = {
  all?: {
    approvedAt: number
  }
  setup?: PersistedTrustedBrowserHookEntry
  archive?: PersistedTrustedBrowserHookEntry
  issueCommand?: PersistedTrustedBrowserHookEntry
  vmRecipe?: PersistedTrustedBrowserHookEntry
}

export type PersistedTrustedBrowserHooks = Record<string, PersistedTrustedBrowserHookRepo>
...
export type AppMemory = UsageValues & {
  ...
  other: UsageValues
  /** Oldest-first memory samples (bytes) for the whole Nexus app; empty before the first snapshot. */
  history: number[]
}
