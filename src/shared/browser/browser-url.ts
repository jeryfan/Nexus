...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === NEXUS_BROWSER_BLANK_URL) {
    return NEXUS_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === NEXUS_BROWSER_BLANK_URL) {
    return null
  }
