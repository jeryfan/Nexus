...
/**
 * Params for creating a Nexus-owned (internal) FileEntry.
 *
...
export type CreateInternalEntryIpcParams =
  | {
      /** Copy the file at `path` into Nexus storage. `name` / `ext` derived from basename+extname. */
      source: 'path'
      path: AbsoluteFilePath
    }
  | {
      /** Download the URL into Nexus storage. `name` / `ext` derived from URL tail, Content-Disposition, and Content-Type. */
      source: 'url'
      url: UrlString
    }
  | {
      /** Decode `data:<mime>;base64,...` and write into Nexus storage. `ext` derived from mime; caller may override the UX display name. */
      source: 'base64'
      data: Base64String
  ...
  | {
      /** Write raw bytes into Nexus storage. No derivation possible — caller is the sole authority for `name` and `ext`. */
      source: 'bytes'
      data: Uint8Array
...
export interface FileIpcApi {
  ...
  /**
   * Create a new Nexus-owned (internal) FileEntry. Always inserts a fresh
   ...
   */
  batchGetDanglingStates(params: {
    ids: FileEntryId[]
  }): Promise<Record<FileEntryId, DanglingState>>
   ...
   */
  batchGetPhysicalPaths(params: {
    ids: FileEntryId[]
  }): Promise<Record<FileEntryId, AbsoluteFilePath>>
