...
> **Task 3 审查修正版**：初版采用三分支条件渲染（maximized / showPanel / 纯对话区），
> 分支切换导致整棵子树重挂载——面板开合会重挂载 AgentThread（丢滚动/焦点/折叠态、
> 整段对话重渲染），最大化会重挂载 ProjectPanel（销毁 webview guest），均为对旧实现
> 的回归（旧实现对话区始终挂载、最大化用 `hidden` 类保留状态）。修正为 Group 常驻、
> 两侧子树永不因开合/最大化而重挂载的方案。
...
4. 面板最大化/还原：在 ≥1280px 宽窗口验证——最大化后面板占满整个内容区、对话区无空白残留；还原后两侧宽度逐像素回到最大化前；浏览器标签页在最大化切换后不重载。目测还原瞬间有无一帧宽度闪烁（命令式 resize 与约束重注册间存在一次绘制机会，已知风险）。
5. 面板收起再打开：宽度保持。
...
10. 文件标签打开时切换树显隐：Monaco 保留光标/undo/滚动（内容未重挂载），
    FileExplorer 滚动位置保持。
11. 树宽持久化三段验证：标签切换（打开文件 ↔ 文件标签）、面板收起再打开、
    重启应用；含「树隐藏状态重启 → 再开树」应恢复拖拽宽度而非 144px。
12. 嵌套组协调：树可见时拖外层面板分隔条（树保持像素宽、内容区吸收）；
    外层面板在 min/max 时拖树分隔条。
13. 极端几何：树拖到 480px 后把外层面板收窄到 240px 附近——树应让位钳制
    （min 120）无溢出；再放宽恢复。
14. Monaco 与图片预览在不同宽度下渲染正常，无多余滚动条。
15. 树分隔条键盘 ←/→ 调整、双击复位到 144px。
16. 最大化 → 收起面板 → 重开面板：应恢复为最大化态（store 标志滞留属预期），布局无破损。

已知边角（接受）：聚焦 agent 页分隔条按 Enter 可折叠对话区（collapsible 副作用），无 UI 反向同步；拖拽分隔条可恢复。
