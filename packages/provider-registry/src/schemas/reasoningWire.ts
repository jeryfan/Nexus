...
export const ReasoningWireValueSchema = z.discriminatedUnion('source', [
  ...
  z.object({ source: z.literal('reasoning-summary') })
])
...
const ReasoningEffortMapSchema = z
  .partialRecord(ReasoningEffortSchema, ReasoningEffortSchema)
  .optional()
...
export const ReasoningWireProfileSchema = z
  ...
  .refine(
    (profile) =>
      profile.disabled === true || profile.default || profile.off || profile.auto || profile.effort,
    {
