...
export function matchReasoningControls(
  ...
  if (budgetRule?.budget)
    controls.push({ kind: 'budget', min: budgetRule.budget.min, max: budgetRule.budget.max })
...
export function matchReasoningTogglePolicy(
  ...
  for (const rule of rules) {
    if (
      (rule.effort !== undefined || rule.toggle !== undefined) &&
      ruleRegex(rule.pattern).test(id)
    ) {
      return rule.toggle
    }
  }
