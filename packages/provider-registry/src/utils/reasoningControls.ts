...
export function deriveLegacyReasoningFields(
  controls: readonly ReasoningControl[]
): DerivedReasoningFields {
  ...
  let supportedEfforts = effort
    ? [...effort.values]
    : toggle
      ? (['none', 'auto'] as const).slice()
      : undefined
  ...
  return {
    ...
    thinkingTokenLimits: budget
      ? {
          min: budget.min,
          max: budget.max,
          ...(budget.default != null ? { default: budget.default } : {})
        }
      : undefined,
