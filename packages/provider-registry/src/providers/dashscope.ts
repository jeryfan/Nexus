...
const qwen38ChatWire: ReasoningWireProfile = modeWire('reasoning_effort', {
  off: 'none',
  effort: EFFORT
})
...
/**
 * SKUs for which Bailian advertises built-in web-search capability, transcribed
 * from the vendor's 支持的模型 matrix (help.aliyun.com/zh/model-studio/web-search).
 * This is provider/endpoint metadata, not a local search execution layer.
 *
 ...
 * so tagging them would advertise a capability the vendor does not support.
 */
 ...
 * web-search capability for the Qwen3.x line only ("Responses API 仅支持 Qwen3.7 Max系列、Qwen3.6、
 * Qwen3.5、qwen3-max"), while these aliases expose it only on Chat Completions. Order Chat first so
 * the default endpoint matches the vendor capability; Responses stays selectable.
 */
 ...
 * Responses-only. The per-endpoint split records where built-in web search is available; everything
 * else inherits the safe Chat Completions default.
 */
