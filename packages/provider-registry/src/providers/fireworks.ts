...
// glm-5-1-fast exists only as a Fireworks router variant (no catalog model row
// and no models.dev fallback entry), so its override must carry a standalone
// name to satisfy the catalog invariant.
const namedOverride = (
  modelId: string,
  name: string,
  support: ReasoningSupport
): Partial<ProviderModelOverride> => ({
  ...override(modelId, support),
  name
})
...
export default defineProvider({
  ...
  overrides: [
    ...
    namedOverride('glm-5-1-fast', 'GLM 5.1 Fast', toggleSupport),
