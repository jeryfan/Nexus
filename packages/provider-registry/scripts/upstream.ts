/**
 * Parse upstream catalog entries (models.dev / OpenRouter) into Nexus metadata, with zod.
 *
...
export type NexusMeta = Partial<
  ...
  >
>
type Caps = NonNullable<NexusMeta['capabilities']>[number]
type Effort = NonNullable<NonNullable<NexusMeta['reasoning']>['supportedEfforts']>[number]

const usd = (n?: number) =>
  n == null ? undefined : { currency: 'USD' as const, perMillionTokens: n }
...
export function parseMdEntry(raw: unknown): NexusMeta | null {
  ...
  const reasoning: NonNullable<NexusMeta['reasoning']> = {}
  const controls: NonNullable<NonNullable<NexusMeta['reasoning']>['controls']> = []
...
export function parseOpenRouterReasoning(raw: unknown): ReasoningSupport | null {
  ...
  const efforts = (descriptor.supported_efforts ?? []).filter((value): value is Effort =>
    VALID_EFFORTS.has(value)
  )
  const selectableEfforts = descriptor.mandatory
  ...
  if (selectableEfforts.length > 0) {
    ...
    !descriptor.supports_max_tokens &&
...
export function parseOrEntry(raw: unknown): NexusMeta | null {
  ...
  if (sp.includes('structured_outputs') || sp.includes('response_format'))
...
function toSupportSpec(
  key: keyof typeof OR_IMAGE_PARAM_KEYS,
  descriptor: OrParamDescriptor
): SupportSpec {
  if (descriptor.type === 'enum') return { type: 'enum', options: descriptor.values }
  if (descriptor.type === 'range')
    return { type: 'range', min: descriptor.min, max: descriptor.max, step: 1 }
...
export function parseOrImageGeneration(raw: unknown): ImageGenerationSupport | null {
  const p = OrEntry.safeParse(raw)
  if (!p.success || Array.isArray(p.data.supported_parameters) || !p.data.supported_parameters)
    return null
  ...
  for (const [wireKey, canonicalKey] of Object.entries(OR_IMAGE_PARAM_KEYS)) {
    ...
    if (parsed.success)
      supports[canonicalKey] = toSupportSpec(
        wireKey as keyof typeof OR_IMAGE_PARAM_KEYS,
        parsed.data
      )
  }
  ...
  if (
    ...
    (outputFormat?.type !== 'enum' ||
      !outputFormat.options.some((value) => value === 'jpeg' || value === 'webp'))
  ) {
...
export function mergeMeta(a: NexusMeta, b: NexusMeta): NexusMeta {
  const out: NexusMeta = { ...a }
  if (b.capabilities)
    out.capabilities = uniqOrdered([
  ...
  if (b.reasoning) {
    ...
    const ef = [
      ...new Set([
        ...(a.reasoning?.supportedEfforts ?? []),
        ...(b.reasoning.supportedEfforts ?? [])
      ])
    ]
...
type Controls = NonNullable<NonNullable<NexusMeta['reasoning']>['controls']>
...
export function finalizeMeta(m: NexusMeta): NexusMeta {
...
export const OpenRouterApiSchema = z
  .object({
    data: z.array(z.object({ id: z.string(), name: z.string().optional() }).loose()).optional()
  })
  .loose()
