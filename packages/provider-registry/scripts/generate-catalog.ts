...
type IndexEntry = { meta: NexusMeta; providers: Set<string> }
...
function buildIndex(md: ModelsDevApi, or: OpenRouterApi): Index {
  ...
  const consider = (id: string, mapped: NexusMeta | null, provider: string) => {
    if (!mapped) return
    const k = canonOf(id)
    if (!k) return
...
async function assignCreators(index: Index, md: ModelsDevApi): Promise<Map<string, string>> {
  ...
  for (const creator of CREATORS) {
    ...
    for (const p of creator.modelsDevProviders)
      for (const id of Object.keys(md[p]?.models ?? {}))
        if (!crossVendorHost(id, creator.id)) ids.add(canonOf(id))
  ...
  for (const creator of CREATORS) {
    ...
    for (const [canonId, e] of index) {
      ...
      if (fam && creator.families.some((f) => fam === f || fam.startsWith(f)))
        claim(canonId, creator.id)
    }
  }
  ...
  for (const creator of CREATORS)
    for (const id of creatorProviderIds.get(creator.id) ?? [])
      if (index.has(id)) claim(id, creator.id)
  return claimed
}
...
function buildModels(index: Index, claimed: Map<string, string>): Map<string, any> {
  ...
  for (const m of models.values()) {
    const kind = /rerank/i.test(m.id)
      ? 'rerank'
      : /embed/i.test(m.id)
        ? 'embedding'
        : creatorKind.get(m.ownedBy)
  ...
  for (const m of models.values()) {
    ...
    if (
      !(m.inputModalities ?? ['text']).includes('text') ||
