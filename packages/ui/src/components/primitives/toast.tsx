...
const createToastStore = () => {
  ...
  const subscribe = (listener: () => void) => {
    ...
    if (listeners.size > 1) {
      console.error(
        'multiple ToastViewport mounted in one window; every toast will render once per host'
      )
    }
    return () => {
...
export const getToastQueue = (): { toasts: ToastRecord[] } => ({
  toasts: defaultToastStore.getSnapshot()
})
...
const getToastA11yProps = (
  type: ToastType
): Pick<React.HTMLAttributes<HTMLDivElement>, 'aria-live' | 'role'> => {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        {toast.title && (
          <div className="break-words font-medium text-sm leading-5">{toast.title}</div>
        )}
        {toast.description && (
          <div className="mt-0.5 break-words text-muted-foreground text-xs leading-5">
            {toast.description}
          </div>
        )}
      </div>
