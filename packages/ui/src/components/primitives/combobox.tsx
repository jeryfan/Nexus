...
import { Button } from '@nexus/ui/components/primitives/button'
import {
  Command,
  CommandEmpty,
  CommandGroup,
  ...
  CommandItem,
  CommandList
} from '@nexus/ui/components/primitives/command'
import { Input } from '@nexus/ui/components/primitives/input'
import {
  Popover,
  PopoverAnchor,
  PopoverContent,
  PopoverTrigger
} from '@nexus/ui/components/primitives/popover'
...
export function Combobox<TExtra extends object = Record<never, never>>({
  ...
  const manualFilterEnabled =
    triggerSearchEnabled || (contentSearchEnabled && Boolean(filterOption))
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
                ...
                <CommandGroup>
                  {visibleOptions.map((option) => (
                    <CommandItem
                      ...
                      className={cn(
                        comboboxItemVariants({ state: option.disabled ? 'disabled' : 'default' })
                      )}
                    >
      ...
      {name && (
        <input
          type="hidden"
          name={name}
          value={multiple ? JSON.stringify(value) : (value as string)}
        />
      )}
    </Popover>
  )
