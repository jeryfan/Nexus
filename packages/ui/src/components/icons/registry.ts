...
// NOTE: the vendor-level regex below duplicate `@nexus/provider-registry`'s
...
// import from `@nexus/provider-registry` directly. When adding / tweaking
...
const PROVIDER_ID_ALIASES: Record<string, string> = {
  ...
  'grok-cli': 'grok',
  // Kimi For Coding is a Moonshot product; reuse the Moonshot mark. Needed even
  // though it groups under `moonshot` — icons resolve by provider id, not by
  // presetProviderId (cf. `zai`, which groups under zhipu but aliases to z-ai).
