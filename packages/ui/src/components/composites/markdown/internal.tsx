/**
 * Static markdown rendering body. Builds the rehype pipeline
 * (defaultRehypePlugins → extended sanitize schema →
...
export interface MarkdownCoreProps {
  ...
  /** Streamdown plugin presets (code / cjk / math). */
...
export function MarkdownCore({
  ...
  const rehypePlugins = useMemo(() => {
    ...
    result.push(
      ...
      [
        rehypePrefixSvgReferences,
        (extendedSchema as { clobberPrefix?: string }).clobberPrefix
      ] as Pluggable,
      harden,
  ...
  const urlTransform = useCallback(
    (value: string, key: string, node: Parameters<typeof defaultUrlTransform>[2]) => {
